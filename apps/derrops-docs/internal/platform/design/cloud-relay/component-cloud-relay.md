...
A cloud-based HTTP proxy service that enables the Derrops API Tester to route requests through a Lambda function in an AWS account instead of making requests directly from the browser.
...
- **`apps/derrops-relay`** — standalone, self-contained NestJS proxy service. Supports three delivery modes:
  - **`direct`** — derrops-cloud calls the relay's `POST /cloud-relay/proxy` synchronously and returns the result inline. Relay must be reachable from derrops-cloud.
  - **`relay-queue`** — derrops-cloud submits jobs to the relay's `POST /cloud-relay/queue`. The relay owns an internal queue (`QueueStore` backend: in-memory by default, SQS + DynamoDB for AWS) and processes jobs asynchronously. derrops-cloud stores a lightweight reference (job ID → relay job ID) and proxies status polls to the relay. Relay must be reachable from derrops-cloud.
  - **`platform-queue`** — the relay **cannot** accept inbound connections (private network). Instead, the relay polls `GET /cloud-relay/queue/next` on derrops-cloud, claims a job, executes it, and posts the result to `POST /cloud-relay/job/:id/result`. derrops-cloud stores the full request payload so the relay can retrieve it on claim.
- **`apps/derrops-cloud`** — hosts the `cloud-relay` module, which manages the registry of relay connections (`cloud_relay_connection` table: name + URL + `delivery_mode` per tenant). Owns the full `cloud_relay_job` entity which tracks every submitted job regardless of delivery mode. The portal always interacts with derrops-cloud; derrops-cloud selects the delivery path based on the connection's `delivery_mode`.
...
- All AWS infrastructure (API Gateway, Cognito User Pool, usage plans, API keys, Lambda authorizer function) provisioned by **CDK in `packages/derrops-infra`**
- The **Cloud Relay Lambda** (`apps/derrops-relay`) is self-contained with its own `infra/` directory — customers can deploy it independently without access to any private Derrops package
...
- Portal UI for managing Cloud Relay connections (name + URL) — backed by `derrops-cloud`
- Request mode selector in the API Tester: **Browser** (current behaviour) or a named **Cloud Relay** instance selected from the connections stored in `derrops-cloud`
...
- Cloud Relay connection settings stored per-tenant in `derrops-cloud` (not in the relay itself)
...
**Iteration 2 — Self-hosted (`apps/derrops-relay` already extracted):**

`apps/derrops-relay` is already a standalone, self-contained application managed as a **git subtree** (same workflow as `apps/derrops-portal/`). It may eventually be open-sourced so customers can review and deploy it themselves.

Key design decisions:

- The relay has **no database** and no connection registry. Connection management lives in `derrops-cloud`.
- In **direct mode** the relay is fully stateless. In **relay-queue mode** the relay owns its job queue via a pluggable `QueueStore` interface — `InMemoryQueueStore` (default) or `SqsQueueStore` (AWS: SQS work channel + DynamoDB for job state). Additional backends can be registered without modifying relay code (same pattern as `SecretStore`). In **platform-queue mode** the relay is fully stateless again — derrops-cloud stores the job state; the relay only polls, executes, and delivers results.
- The relay is authenticated via `RELAY_API_KEY` (inbound: validates calls from derrops-cloud) and `RELAY_PLATFORM_TOKEN` (outbound: sent as Bearer when polling derrops-cloud in platform-queue mode). Both values are generated when a connection is created in derrops-cloud and configured on the relay as env vars.
...
- Configuration relies **solely on environment variables** — no `@derrops/config` module, minimising dependencies for self-hosted deployments
...
├── app/         # NestJS core — cloud-agnostic (env-vars config, no @derrops/config)
...
└── infra/       # Self-contained deployment infrastructure (no dependency on packages/derrops-infra)
...
The `infra/` directory at the root of `apps/derrops-cloud-relay` is the **only** infrastructure customers need to self-host the Relay. It has no dependency on `packages/derrops-infra`, which is private to the Derrops platform and is not distributed. Customers clone the relay repository, configure environment variables, and run `cd infra/aws && cdk deploy` (or equivalent for their cloud) — no access to any Derrops private package is required.

### Relationship to Existing Components

The portal **always** submits requests through derrops-cloud. derrops-cloud selects the delivery path based on the connection's `delivery_mode`. Three modes are supported to accommodate any customer network topology.

**`direct` mode** — relay is publicly reachable from derrops-cloud (e.g. Derrops-managed Lambda behind API Gateway):

```mermaid
graph TD
    Portal[Derrops Portal\nAPI Tester] -->|POST /cloud-relay/job| DerropsCloud[derrops-cloud\ncloud-relay module]
    DerropsCloud -->|POST /cloud-relay/proxy\nBearer api_key| RelayDirect[apps/derrops-relay\nPOST /cloud-relay/proxy]
    RelayDirect -->|HTTP proxy| ExternalAPI[External API]
    RelayDirect -->|response| DerropsCloud
    DerropsCloud -->|store completed job| JobStore[(cloud_relay_job\nstatus=completed)]
    DerropsCloud -->|result inline| Portal

    style RelayDirect fill:#ff6b6b,stroke:#333,stroke-width:3px
    style DerropsCloud fill:#82b366,stroke:#333,stroke-width:2px
```

**`relay-queue` mode** — relay is in a private network but reachable from derrops-cloud (server-to-server), not from the browser:

```mermaid
graph TD
    Portal[Derrops Portal\nAPI Tester] -->|POST /cloud-relay/job| DerropsCloud[derrops-cloud\ncloud-relay module]
    DerropsCloud -->|store pending job| JobRef[(cloud_relay_job\nrelay_job_id ref)]
    DerropsCloud -->|POST /cloud-relay/queue\nBearer api_key| RelayAsync[apps/derrops-relay\nqueue backend]
    ...
    Portal -->|GET /cloud-relay/job/:id| DerropsCloud
    DerropsCloud -->|GET /cloud-relay/job/:relayJobId\nBearer api_key| RelayAsync
    RelayAsync -->|job state + result| DerropsCloud
    DerropsCloud -->|status + result| Portal
...
**`platform-queue` mode** — relay is fully isolated (private network, no inbound connections). The relay polls derrops-cloud outbound:

```mermaid
graph TD
    Portal[Derrops Portal\nAPI Tester] -->|POST /cloud-relay/job| DerropsCloud[derrops-cloud\ncloud-relay module]
    DerropsCloud -->|store pending job + full request| JobStore[(cloud_relay_job\nstatus=pending\nrequest=JSONB)]
    Portal -->|GET /cloud-relay/job/:id| DerropsCloud
    DerropsCloud -->|job status + result| Portal

    RelayPlatform[apps/derrops-relay\nQueuePollerService] -->|GET /cloud-relay/queue/next\nBearer platform_token| DerropsCloud
    DerropsCloud -->|claimed job + request payload| RelayPlatform
    RelayPlatform -->|execute| ExternalAPI[External API]
    RelayPlatform -->|POST /cloud-relay/job/:id/result\nBearer platform_token| DerropsCloud
    DerropsCloud -->|update job\nstatus=completed| JobStore
...
```mermaid
graph TB
    subgraph "Derrops Portal (Browser)"
    ...
    subgraph "derrops-cloud (NestJS / Lambda)"
...
`undici` is the recommended HTTP client for server-side proxy use in Node.js 22+ (it's the underlying engine of `fetch` in Node.js). No additional external dependencies needed — NestJS, TypeORM, class-validator, and `@derrops/config` are all already present.
...
```mermaid
graph LR
    Private["@derrops/private"] --> Cloud[derrops-cloud]
    Config["@derrops/config"] --> Cloud
    Undici[undici] --> Cloud
    Cloud -->|OpenAPI client generation| Portal[derrops-portal]
```
...
FUNCTION proxy(dto: CloudProxyRequest): CloudProxyResponse | CloudProxyError
  ...
  IF responseBody.length > maxBody:
    ...
    responseHeaders['X-Derrops-Truncated'] = 'true'
...
- **Authentication — Custom Lambda Authorizer**: The Cloud Relay routes on API Gateway are protected by a custom Lambda authorizer (not the built-in Cognito authorizer). The authorizer:
  ...
  2. Validates the JWT against the Derrops Cognito User Pool (signature, expiry, audience)
...
```typescript
// app/src/policy/types.ts  (apps/derrops-cloud-relay, cloud-agnostic core)
...
The Cloud Relay is a module within `derrops-cloud`, which is already deployed as an AWS Lambda behind API Gateway. No separate deployment is needed for the default Derrops-managed instance.

For self-hosted or customer-VPC instances, deploy `derrops-cloud` as a standalone Lambda:

```bash
# From monorepo root
pnpm --filter @derrops/cloud run build
# Then deploy via derrops-backend / Amplify as usual
```
...
### Configuration Options (derrops-cloud module)
...
All values defined in `packages/derrops-config/src/config.ts` following the no-magic-numbers convention.

### New Environment Variables (derrops-cloud-relay — Iteration 2)

The self-hosted `apps/derrops-cloud-relay` application uses **only environment variables** for configuration (no `@derrops/config` module dependency):
...
For the Derrops-managed instance embedded in `apps/derrops-cloud`, configuration continues to use `@derrops/config` in the module wrapper.

## Testing Strategy

### Unit Tests (derrops-cloud)
...
### Integration Tests (derrops-cloud)
...
The Cloud Relay has two distinct deployment models with separate infrastructure ownership. The split is important because `packages/derrops-infra` is **private to the Derrops platform** and is never distributed to customers.
...
| **Custom Lambda Authorizer** (function + CDK wiring) | `packages/derrops-infra`   | AWS CDK Lambda construct                       | **Infrastructure** — shared across the entire environment; must not be gated behind a feature deploy             |
| **Cloud Relay Lambda** (function definition)         | `packages/derrops-backend` | Amplify `defineFunction`                       | **Application** — independently deployable as a feature; follows the same pattern as the existing `api` function |

`packages/derrops-infra` is used **only** for the Derrops platform-managed deployment (Iteration 1). It is a private package and is not part of the customer-facing relay distribution.

#### Why the authorizer lives in infra, not backend

The Lambda authorizer is **environment-wide shared infrastructure**. Every request to any Cloud Relay endpoint passes through it. Deploying it via Amplify would tie its availability to the app deployment cycle, creating a risk of the gateway becoming unauthenticated during a failed or in-progress feature deploy. By wiring it in CDK (`derrops-infra`) it is deployed once, independently of application code, and remains stable while features come and go.
...
```mermaid
graph TB
    subgraph "packages/derrops-infra (CDK) — platform private"
        APIGW[API Gateway\nCloudRelayApiStack]
    ...
    subgraph "packages/derrops-backend (Amplify) — platform private"
        CloudFn[Cloud Relay\ndefineFunction]
    end

    subgraph "apps/derrops-cloud (NestJS)"
        CloudModule[cloud-relay module\napp logic + HAR proxy]
        AuthorizerCode[authorizer/\nfunction code]
    end

    CloudFn -->|entry: derrops-cloud/src/lambda.ts| CloudModule
    AuthLambdaInfra -->|entry: derrops-cloud/src/authorizer.ts| AuthorizerCode
    APIGW -->|Lambda proxy integration - ARN imported from Amplify output| CloudFn
```

The authorizer function **code** lives in `apps/derrops-cloud/src/authorizer.ts` (co-located with the rest of the cloud app), but it is **wired into API Gateway by CDK in `derrops-infra`**, not by Amplify. This mirrors the existing pattern where the main API Lambda is defined in `derrops-backend` but its ARN is imported into `ApiStack` via a CloudFormation export.
...
```bash
cd apps/derrops-cloud-relay/infra/aws
cdk deploy
```

No Derrops private packages, no `packages/derrops-infra`, no Amplify dependency.

#### Infrastructure diagram (Iteration 2 — self-hosted)

```mermaid
graph TB
    subgraph "apps/derrops-cloud-relay (customer repo)"
...
### derrops-cloud (existing package, new module added)

No new package — the `cloud-relay` module is added to the existing `apps/derrops-cloud` NestJS application:

```
apps/derrops-cloud/src/
└── cloud-relay/
...
```bash
pnpm --filter @derrops/cloud run migration:generate -- src/migrations/CreateCloudRelayConnection
```

### derrops-infra (CDK — Iteration 1, platform-managed only)

> **Note**: This section covers the Derrops platform-managed deployment (Iteration 1). `packages/derrops-infra` is a **private** package — it is never distributed to customers. For the self-hosted deployment (Iteration 2), see the `apps/derrops-cloud-relay/infra/` section below.

A new CDK stack is added to `packages/derrops-infra` for the platform-managed Cloud Relay infrastructure. It follows the same pattern as the existing `ApiStack` (which imports the main API Lambda ARN from Amplify and wires it to API Gateway).

```
packages/derrops-infra/lib/stack/
...
```typescript
// packages/derrops-infra/lib/stack/cloud-relay.ts (sketch)
...
export class CloudRelayStack extends Stack {
  constructor(scope: Construct, id: string, props: CloudRelayStackProps) {
    ...
    //    Code lives in derrops-cloud/src/authorizer.ts — same repo, deployed here by CDK
    const authorizerFn = new NodejsFunction(this, 'CloudRelayAuthorizer', {
      entry: '<derrops-cloud>/src/authorizer.ts',
      environment: {
    ...
    const api = new RestApi(this, 'CloudRelayApi', {
      restApiName: 'Derrops Cloud Relay',
    ...
    // 6. Cloud Relay Lambda (ARN imported from Amplify derrops-backend output)
    ...
    new CfnOutput(this, 'CloudRelayApiUrl', {
      value: api.url,
      exportName: 'DerropsCloudRelayApiUrl',
    })
  }
}
```

Register the new stack in `packages/derrops-infra/bin/cdk.ts` alongside the existing stacks.
...
### apps/derrops-cloud-relay/infra/ (Iteration 2 — self-hosted, self-contained)

The self-hosted relay ships its own infrastructure at the root of the `apps/derrops-cloud-relay` package. This has **no dependency on `packages/derrops-infra`** or any other private Derrops package. It is the only infrastructure a customer needs.
...
│   │   ├── cdk.json
│   │   └── package.json         # Standalone — no derrops-infra dependency
...
The AWS CDK stack in `infra/aws/` is standalone — customers `cd infra/aws && cdk deploy` from the relay directory. It provisions its own API Gateway, Lambda authorizer, and usage plans. It does not import any CloudFormation exports from `packages/derrops-infra`.

```typescript
// apps/derrops-cloud-relay/infra/aws/lib/relay-stack.ts (sketch)
...
export class RelayStack extends Stack {
  constructor(scope: Construct, id: string, props: RelayStackProps) {
    ...
    // no imports from derrops-infra, no @derrops/config dependency.
    ...
    const relayFn = new NodejsFunction(this, 'RelayFn', {
      ...
      environment: {
        ...
        // All config via env vars — no @derrops/config
      },
...
### derrops-backend (Amplify — Cloud Relay Lambda function)

A new `defineFunction` entry is added in `packages/derrops-backend/amplify/functions/` for the Cloud Relay Lambda. This follows the exact same pattern as the existing `api` function:

```
packages/derrops-backend/amplify/functions/
├── api/
...
```typescript
// packages/derrops-backend/amplify/functions/cloud-relay/resource.ts
...
const derropsCloudRoot = path.dirname(require.resolve('@derrops/cloud/package.json'))

export const CloudRelay = defineFunction({
  name: config['app.cloud-relay.name'],
  entry: path.join(derropsCloudRoot, 'src', 'lambda.ts'), // same Lambda entry; NestJS routes to the module
...
The Amplify deploy outputs the Lambda ARN as a CloudFormation export. `CloudRelayStack` in `derrops-infra` imports that ARN (the same mechanism used by the existing `ApiStack` for the main API Lambda).

## Documentation Requirements

- [ ] Update `apps/derrops-docs/docs/components.md` to include Cloud Relay
- [ ] Add `apps/derrops-docs/docs/cloud-relay.md` with user guide (how to add a connection, use cases)
- [ ] Add security note about credential handling to the user guide
- [ ] Update `apps/derrops-cloud/CLAUDE.md` with the new module and `src/authorizer.ts` entry point
- [ ] Update `packages/derrops-infra/README.md` with `CloudRelayStack` (resources, outputs, deploy order) — platform-managed deployment only
- [ ] Update `packages/derrops-backend/README.md` with the `cloud-relay` function and its dependency on the infra stack ARN export
- [ ] Add `apps/derrops-cloud-relay/infra/README.md` documenting the self-hosted deployment for each cloud (`aws/`, `azure/`, `gcp/`) and Docker

## Rollout Plan

### Phase 1: App Logic (Iteration 1a)

- [ ] Add `cloud-relay` NestJS module to `derrops-cloud` (controller, ProxyService, connection CRUD, DTOs, entity)
- [ ] Write Lambda authorizer function at `apps/derrops-cloud/src/authorizer.ts` (JWT validation + API key lookup)
...
- [ ] Add `CloudRelayStack` to `packages/derrops-infra` (API Gateway, usage plans, token authorizer wired to authorizer Lambda)
- [ ] Add `cloud-relay` function definition to `packages/derrops-backend/amplify/functions/`
...
- [ ] Extract `cloud-relay` NestJS module into `apps/derrops-cloud-relay` (git subtree)
- [ ] Build `infra/aws/` CDK stack at the root of `apps/derrops-cloud-relay` — standalone, no `packages/derrops-infra` dependency
...
- [ ] Ensure all config uses environment variables only — no `@derrops/config`, no private Derrops packages
- [ ] Support for multiple simultaneous Cloud Relay regions
- [ ] Request history tied to Cloud Relay source
- [ ] Validate customer self-hosting end-to-end: `cd infra/aws && cdk deploy` with zero Derrops private dependencies
...
### Alternative 2: Standalone `apps/derrops-cloud-relay` (Chosen for Iteration 2)

Deploy the proxy as a standalone application in `apps/derrops-cloud-relay`, managed as a git subtree (same workflow as `apps/derrops-portal/`). `apps/derrops-cloud` embeds it by importing the NestJS module, keeping the Derrops-managed deployment as a single Lambda artifact while enabling customer self-hosting.
...
**Why not chosen for iteration 1:** Adding a module to the existing `derrops-cloud` NestJS app is the lowest-friction path. The module is intentionally isolated (own module, own controller, no cross-module dependencies) so the Iteration 2 extraction is low-risk.

**Extraction plan (Iteration 2):**

- `cloud-relay` moves to `apps/derrops-cloud-relay/app/` (NestJS, cloud-agnostic, env-vars config only)
- Cloud-specific secret store implementations land in `app-aws/`, `app-azure/`, `app-cp/`
- `apps/derrops-cloud` imports `CloudRelayModule` from the subtree — no breaking change to the Derrops-managed deployment
...
- The Derrops-managed instance continues to use the Derrops Cognito pool and existing usage plans, keeping backward compatibility
...
- [ ] Relay validates vendor job signature against vendor JWKS (`DERROPS_VENDOR_JWKS_URL`)
...
- [ ] Vendor JWKS endpoint (`GET /cloud-relay/.well-known/jwks.json`) on `derrops-cloud`
...
- [ ] API Gateway + custom Lambda authorizer + Cognito usage plans (Derrops-managed relay)
