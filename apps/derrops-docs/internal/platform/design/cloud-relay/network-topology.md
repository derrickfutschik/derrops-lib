...
The Derrops edge consists of three runtime components: **derrops-cloud** (vendor SaaS), **Cloud Relay** (`apps/derrops-relay`), and **Aegis** (`apps/derrops-aegis`). The relay and Aegis are customer-deployed and are intentionally kept as separate, independently deployable artifacts. This document explains the topology options, why the separation exists, and the security properties it provides.
...
The `delivery_mode` on a relay connection determines how derrops-cloud routes requests. The choice is a direct function of what the customer's network topology allows.

```mermaid
flowchart LR
    subgraph Derrops["Derrops Platform"]
        Cloud[derrops-cloud]
    end
    ...
    Browser[Browser] -->|always via derrops-cloud| Cloud
...
derrops-cloud calls the relay's `POST /cloud-relay/proxy` synchronously and waits for the response.

**Network requirement**: relay must accept inbound HTTPS from derrops-cloud.
**Use when**: relay is publicly reachable (e.g. Lambda behind API Gateway, Docker on a public host).

### `relay-queue`

derrops-cloud submits a job to the relay's `POST /cloud-relay/queue`. The relay's internal `QueueStore` (in-memory or SQS + DynamoDB) processes it asynchronously. derrops-cloud polls the relay for the result.

**Network requirement**: relay must accept inbound HTTPS from derrops-cloud. Browser does not need to reach the relay.
**Use when**: the relay is on a private network reachable from derrops-cloud (server-to-server), but not from the browser directly.

### `platform-queue`

The relay polls `GET /cloud-relay/queue/next` on derrops-cloud, claims a job, executes it, and posts the result to `POST /cloud-relay/job/:id/result`. The relay makes **only outbound** calls.

**Network requirement**: relay needs outbound HTTPS to derrops-cloud only. No inbound connections required at all.
...
```
Can the relay accept inbound connections from derrops-cloud?
...
The security model of the system is: **neither derrops-cloud nor Aegis alone can cause the relay to execute a job**.
...
1. **Vendor JWT** — signed by derrops-cloud's private key. Proves the job originated from the Derrops platform. Scoped to this specific relay (`aud` = `RELAY_ID`).
...
If the Derrops platform is compromised, it cannot execute relay jobs unilaterally — it cannot forge the customer's Aegis signature. If Aegis is compromised, it cannot execute jobs unilaterally — it cannot forge the vendor's signature. Both must be valid for the relay to proceed.
