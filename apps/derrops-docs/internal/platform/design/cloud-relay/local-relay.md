...
The Derrops API Tester routes requests through a Cloud Relay to solve CORS, egress IP, credential exposure, and timing problems. However, when a developer is running a service **locally on their machine** (e.g. `http://localhost:3001`), no cloud-hosted or VPC-deployed relay can reach that address.

The solution is for the developer to run the `derrops-relay` process locally. This is architecturally identical to any other relay deployment — the relay happens to run on the developer's machine with target services also on localhost.
...
The developer runs the existing `derrops-relay` process on their own machine. This is architecturally identical to any customer-deployed relay — it just happens to run on `localhost:PORT` with target services also on localhost.

### Why SQS instead of HTTP polling

When the relay cannot accept inbound connections (developer behind NAT or a corporate firewall), it needs a pull-based channel. Rather than the relay HTTP-polling a derrops-cloud endpoint, each relay registration provisions a **dedicated AWS SQS queue**. derrops-cloud publishes jobs to the queue; the relay consumes via SQS long-poll.

This gives:

- **No inbound ports** — the relay only makes outbound connections (SQS + derrops-cloud for result delivery)
...
- **Works from any network** — outbound HTTPS to SQS and derrops-cloud is sufficient
...
1. `derrops relay init` authenticates the developer via Cognito (PKCE), stores relay configuration in `~/.derrops/config`, and stores only the **Cognito tokens** in `~/.derrops/credentials`.
2. `derrops relay start` reads configuration from `~/.derrops/config` and tokens from `~/.derrops/credentials`, then exchanges the `id_token` for **temporary AWS credentials** (AccessKeyId + SecretKey + SessionToken, valid ~1 hour) via the Cognito Identity Pool → STS `AssumeRoleWithWebIdentity`.
...
5. The `refresh_token` is valid for **30 days**. When it expires, the developer runs `derrops relay init --force` to re-authenticate.
...
```
arn:aws:sqs:*:*:derrops-${aws:PrincipalTag/tenantId}-local-${aws:PrincipalTag/userId}-*
```
...
```mermaid
sequenceDiagram
    participant Portal as Derrops Portal (browser)
...
Relay connections in derrops-cloud carry a `type` discriminator:
...
A `local-dev` relay has additional constraints enforced by derrops-cloud:
...
- The developer is already authenticated to derrops-cloud via Cognito
- Running a local Aegis instance is significant operational overhead for a dev workflow
- The relay's `AEGIS_REQUIRED` env var defaults to `false` when started via `derrops relay start`
...
- Warns the developer if the selected target URL begins with `localhost` or `127.0.0.1` but no `local-dev` relay is registered:
  ...
  > _Your target is a localhost URL. Start a local relay with `derrops relay start` to route this request._

---

## Developer Onboarding: `derrops-cli`

The `derrops-cli` package (`apps/derrops-cli`) is a dedicated CLI tool for developer-facing workflows, implemented with oclif. It depends on `derrops-relay` as a workspace dependency to start the relay programmatically.

The local relay workflow uses two commands:

### `derrops relay init` (one-time setup)
...
**`~/.derrops/config`** (mode `0644`) — non-sensitive relay configuration:
...
**`~/.derrops/credentials`** (mode `0600`) — sensitive time-limited tokens:
...
Both files follow the same profile structure (`[default]`, `[staging]`, etc.). Multiple profiles let developers connect to different Derrops tenants or environments simultaneously:
...
Reads configuration from `~/.derrops/config` and tokens from `~/.derrops/credentials`, refreshes the Cognito tokens if expired, and starts the relay:

```
$ derrops relay start

  ✓ Local relay starting (relay_id: abc-123)
  ✓ Connecting via SQS → https://sqs.ap-southeast-2.amazonaws.com/123456789/derrops-acme-local-abc123-relay456
...
- **Secrets stay on the developer's machine.** Credentials are resolved inside the local process and never sent to the browser or to derrops-cloud in plaintext.
...
- **Works behind NAT / corporate firewalls.** Any network with outbound HTTPS access to derrops-cloud is sufficient.
