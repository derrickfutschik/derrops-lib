...
See [Conventions](./conventions) for domain responsibilities and the full tag specification.
...
| `obs`             | `ingestion`       | Planned   | `derrops--obs--ingestion`                 | HTTP event ingestion API endpoint                |
| `obs`             | `enrichment`      | Planned   | `derrops--obs--enrichment`                | OASpec-based log enrichment pipeline Lambda      |
| `obs`             | `storage`         | Planned   | `derrops--obs--storage`                   | Per-tenant log S3 buckets and Aurora tables      |
| `obs`             | `query`           | Planned   | `derrops--obs--query`                     | Log retrieval, aggregation, and export           |
| `obs`             | `alerts`          | Suggested | `derrops--obs--alerts`                    | Alert rule evaluation and notification fanout    |
...
```typescript
const obs = orgConvention.with({ domain: 'obs' })

const ingestion = obs.with({ service: 'ingestion' })
const enrichment = obs.with({ service: 'enrichment' })
const storage = obs.with({ service: 'storage' })
const query = obs.with({ service: 'query' })
const alerts = obs.with({ service: 'alerts' })
```

Per-tenant log storage:

```typescript
// {region}--{env}--derrops--{tenantId}--obs--storage--logs
obs.name({ type: 's3Bucket', service: 'storage', tenant: tenantId, key: 'logs' })
```
...
| `storage`    | 🔲      | Per-tenant S3 `{region}--{env}--derrops--{tenantId}--obs--storage--logs`, Aurora tables |
...
2. Pick the domain (see [Conventions](./conventions) if unsure).
...
- No domain prefix in the service name — the domain is already a separate segment (`obs.storage`, not `obs.obs-storage`).
...
1. Propose the new domain in this registry and [Conventions](./conventions).
2. Add the domain tag to `apps/derrops-docs/internal/platform/design/tags.yml`.
3. Update the `tagKeys` on the org-level convention instance in `packages/derrops-config/src/derrops-conventions.ts`.
