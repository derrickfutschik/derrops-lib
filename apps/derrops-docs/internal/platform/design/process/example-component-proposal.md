...
A real-time metrics aggregation engine that processes Derrops log events and computes rolling statistics (latency percentiles, error rates, throughput) for API endpoints.

### Problem Statement

Currently, the Derrops platform stores raw HTTP request/response logs, but users must manually query and aggregate these logs to understand API performance trends. This is inefficient for:
...
This component sits between the log ingestion pipeline and the dashboard/API layer, consuming events from `@derrops/client` connectors and providing aggregated data to the portal.

```mermaid
graph TD
    A[Client Apps] --> B["@derrops/client-*"]
    B --> C[Log Ingestion API]
    C --> D[Raw Logs Database]
    C --> E[Metrics Aggregator]
    E --> F[Aggregated Metrics DB]
    F --> G[Derrops Portal Dashboard]
    F --> H[Alerts Engine]
...
```typescript
import { MetricsAggregator } from '@derrops/metrics-aggregator'
...
```typescript
import { computeMetrics, calculatePercentile } from '@derrops/metrics-aggregator'
...
```json
{
  "logDatabaseUrl": "postgresql://user:pass@host:5432/derrops_logs",
  "metricsDatabaseUrl": "postgresql://user:pass@host:5432/derrops_metrics",
...
```mermaid
graph TD
    Core[@derrops/private] --> Aggregator[@derrops/metrics-aggregator]
    Lib[@derrops/public] --> Aggregator
...
pnpm --filter @derrops/metrics-aggregator add @derrops/metrics-aggregator

# Or install as a standalone service
pnpm add @derrops/metrics-aggregator
```
...
```json
{
  "name": "@derrops/metrics-aggregator",
  "version": "0.1.0",
  "type": "module",
  "main": "./dist/index.cjs",
...
```mermaid
graph LR
    A[@derrops/private] --> C[@derrops/metrics-aggregator]
    B[@derrops/public] --> C
```
...
```bash
pnpm --filter @derrops/private run build &&
pnpm --filter @derrops/public run build &&
pnpm --filter @derrops/metrics-aggregator run build
```
