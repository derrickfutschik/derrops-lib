...
1. **Catalogue adopt** — search the Derrops-managed global OpenSearch catalogue and adopt a platform-managed API.
...
This is a new endpoint on `derrops-cloud`. It must be added to the NestJS controller and included in the OpenAPI spec so the portal client can be regenerated.
...
**Config:** Timeout and redirect policy live in `derrops-config`, not hardcoded.
