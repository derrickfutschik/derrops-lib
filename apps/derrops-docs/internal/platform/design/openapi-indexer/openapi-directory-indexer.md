...
- Indexing into OpenSearch Index (`derrops-openapis`)
...
- Search service in derrops-cloud for querying indexed APIs
...
1. **Lambda Function** in `packages/derrops-backend/amplify/functions/` - handles S3 event processing and indexing
2. **Search Service** in `apps/derrops-cloud/` - provides query API for indexed specifications

```mermaid
graph TD
    S3[S3 Bucket<br/>APIs-guru Structure] -->|S3 Event| Lambda[OpenAPI Indexer Lambda]
    Lambda -->|Parse & Index| OS[OpenSearch Serverless<br/>derrops-collection]

    subgraph "apps/derrops-cloud"
    ...
    Service -->|Query| OS
    Portal[Derrops Portal] -->|REST API| Controller
...
| `IndexResult`           | Indexing result            | `packages/derrops-backend/amplify/functions/openapi-indexer/` |
| `OpenApiSearchQuery`    | Search request             | `apps/derrops-cloud/src/openapi-search/dto/`                  |
| `OpenApiSearchResponse` | Search response            | `apps/derrops-cloud/src/openapi-search/dto/`                  |
...
graph TB
    ...
    subgraph "packages/derrops-backend"
    ...
    subgraph "@derrops/infra"
        OpenSearch[OpenSearch Serverless<br/>derrops-collection]
    end
...
```typescript
// packages/derrops-backend/amplify/functions/openapi-indexer/handler.ts
...
```typescript
// apps/derrops-cloud/src/openapi-search/openapi-search.controller.ts
...
```typescript
// apps/derrops-cloud/src/openapi-search/openapi-search.service.ts
...
```bash
# Free-text search
curl "https://api.derrops.com/openapi-search?query=user+authentication"

# Filter by provider
curl "https://api.derrops.com/openapi-search?provider=github.com"

# Combined filters
curl "https://api.derrops.com/openapi-search?tags=payments&method=POST&size=20"

# Get specific spec
curl "https://api.derrops.com/openapi-search/github.com/api.github.com/1.1.4"

# List providers
curl "https://api.derrops.com/openapi-search/providers"
```
...
### Search Service Dependencies (derrops-cloud)
...
### S3 Bucket Setup (in @derrops/infra)

Add the S3 bucket for OpenAPI specs:

```typescript
// packages/derrops-infra/lib/stack/openapi-bucket-stack.ts
...
export class OpenApiBucketStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const bucket = new s3.Bucket(this, 'OpenApiSpecsBucket', {
      bucketName: 'derrops-openapi-specs',
      versioned: true,
      encryption: s3.BucketEncryption.S3_MANAGED,
    })

    // Lambda will be created in derrops-backend
...
```typescript
// packages/derrops-backend/amplify/functions/openapi-indexer/resource.ts
...
### Adding the Search Module to derrops-cloud

```typescript
// apps/derrops-cloud/src/openapi-search/openapi-search.module.ts
...
- [ ] Usage examples in derrops-docs
...
- [ ] Create S3 bucket in @derrops/infra
...
- [ ] Implement search service in derrops-cloud
