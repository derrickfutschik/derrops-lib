...
This section covers the design of the OASpec domain: how APIs and their OpenAPI specifications are stored, indexed, versioned, and searched across the Derrops platform.
...
graph TD
    ...
    subgraph S3Store["S3 — Raw Spec Storage"]
        SpecBucket["{region}--{env}--derrops--{tenantId}--oaspec--storage--specs\nFull YAML/JSON spec files"]
    end

    subgraph OS["OpenSearch — Search & Telemetry"]
        SpecIdx["derrops--{tenantId}--oaspec--spec\nSpec metadata per version"]
        SvrIdx["derrops--{tenantId}--oaspec--server\nServer entries per version"]
        OpIdx["derrops--{tenantId}--oaspec--operation\nOperation entries per version"]
        ParamIdx["derrops--{tenantId}--oaspec--param\nParameter entries"]
        ModelIdx["derrops--{tenantId}--oaspec--model\nSchema models per version"]
    end
