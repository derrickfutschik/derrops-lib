...
A self-contained, stateless HTTP proxy service. The Derrops platform connects to it; the portal selects which relay instance to use. The relay's only job is to receive a HAR-format proxy request, resolve secrets and template expressions, enforce SSRF-safe security policies, execute the request, and mask secrets from the response.

**Connection management** (which relay instances exist, their URLs) lives in `apps/derrops-cloud`, not here. The relay has no database and no connection registry.

Designed to be deployed independently — no dependencies on any other Derrops packages.
...
The relay exposes a single endpoint. Connection management is handled by `apps/derrops-cloud`.
