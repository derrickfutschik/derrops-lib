...
Aegis is the Derrops policy engine. It runs in your infrastructure and gives you control over what the relay is allowed to do — without any of your credentials or secrets ever leaving your environment.
...
- Docker
- Your Derrops API key (from Portal → Settings → API Keys)
...
```bash
AEGIS_URL=http://derrops-aegis:3200   # URL of your Aegis service
...
docker run -d \
  ...
  -e AEGIS_URL=http://derrops-aegis:3200 \
...
3. Enter the URL where your Aegis service is accessible from the Derrops cloud (e.g. `https://aegis.internal.example.com`).
4. Click **Save**.
...
services:
  derrops-aegis:
    image: derrops/aegis:latest
    restart: unless-stopped
    environment:
      AEGIS_PLATFORM_API_KEY: ${DERROPS_API_KEY}
      AEGIS_SIGNING_KEY_PATH: /keys/aegis-private.pem
      AEGIS_IDP_ISSUER: https://your-idp.example.com
      AEGIS_IDP_AUDIENCE: derrops
    volumes:
  ...
  derrops-relay:
    ...
    environment:
      ...
      AEGIS_URL: http://derrops-aegis:3200
