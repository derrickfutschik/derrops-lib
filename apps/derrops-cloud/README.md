...
NestJS backend API for the Derrops platform. Runs as a standalone server in development and as an AWS Lambda function in production.
...
pnpm --filter @derrops/cloud install
```
...
pnpm --filter @derrops/cloud run start:dev

# Production mode
pnpm --filter @derrops/cloud run start:prod
```

### OpenAPI Spec and Client Generation

```bash
pnpm --filter @derrops/cloud run generate:openapi
```

Generates the OpenAPI spec to `dist/openapi.json` and `src/openapi.json`.

```bash
pnpm --filter @derrops/cloud run generate:client
```

Generates a typed Axios client to `apps/derrops-portal/src/client/derrops-cloud`.
...
```bash
# Unit tests
pnpm --filter @derrops/cloud run test

# Watch mode
pnpm --filter @derrops/cloud run test:watch

# Coverage
pnpm --filter @derrops/cloud run test:cov
```

### Linting

```bash
pnpm --filter @derrops/cloud run lint
```
...
See the [Cloud Relay Component Design](../derrops-docs/notes/proposals/cloud-relay/component-cloud-relay.md) for the full architecture, sequence diagrams, and design decisions.

The cloud relay subsystem (`relay-instance/`, `aegis-instance/`, `cloud-relay/`, `vendor-jwt/`) implements the Derrops relay network control plane:
...
Database infrastructure (Aurora Serverless PostgreSQL) and API Gateway are defined in `packages/derrops-infra/`. Lambda deployment is managed via `packages/derrops-backend/` (AWS Amplify Gen 2).

See [packages/derrops-infra/README.md](../../packages/derrops-infra/README.md) and [packages/derrops-backend/README.md](../../packages/derrops-backend/README.md) for infrastructure setup.
...
PORT=3001
NODE_ENV=production
CORS_ORIGIN=https://portal.derrops.com
...
DERROPS_VENDOR_SIGNING_KEY_JWK={"kty":"EC","crv":"P-256",...}
```
