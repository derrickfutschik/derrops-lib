...
export class CloudRelayConnection {
  ...
  @ApiProperty({
    ...
    default: 'managed',
    description:
      'managed     — Derrops-hosted Lambda relay.\n' +
  ...
  @ApiProperty({
    ...
    description:
      'direct         — derrops-cloud calls relay synchronously. Relay must be reachable from derrops-cloud.\n' +
      'relay-queue    — derrops-cloud submits to relay queue, polls relay for result. Relay must be reachable from derrops-cloud.\n' +
      'platform-queue — relay polls derrops-cloud outbound and posts results back. Use when relay cannot accept inbound connections.\n' +
  ...
  @ApiPropertyOptional({
    enum: ['platform', 'relay'],
    description:
      'platform — Derrops provisions and owns the SQS FIFO queue (default for local-dev). ' +
      'relay    — Customer provisions the queue in their own AWS account and grants the DerropsSqsPublishRole SendMessage access. ' +
      "Use relay mode when the customer's network cannot reach SQS endpoints in the Derrops account.",
  })
  ...
  @ApiPropertyOptional({
    description:
      ...
      'platform mode: provisioned by derrops-cloud and stored here. ' +
      'relay mode: provided by the customer at registration time.',
    example:
      'https://sqs.ap-southeast-2.amazonaws.com/123456789/derrops-acme-local-abc123-relay456.fifo',
  })
  ...
  @ApiProperty({
    description:
      ...
      'direct/relay-queue: derrops-cloud sends this as Bearer token to the relay. ' +
      'platform-queue: relay sends this as Bearer token when polling derrops-cloud. ' +
