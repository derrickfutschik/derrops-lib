 ...
 *
 * platform (default):
 *   derrops-cloud creates the queue in the Derrops AWS account at relay
 *   registration time and deletes it when the connection is removed.
 *   Queue name convention: derrops--{tenantId}--relay--middleware--{connId}.fifo
 ...
 *   They provide the queue URL at registration and grant the DerropsSqsPublishRole
 *   sqs:SendMessage access via a queue resource policy.
 *   derrops-cloud publishes cross-account using that role.
 *   The relay uses its own IAM credentials (instance profile, role, etc.) to consume.
 *   derrops-cloud does NOT create or delete the queue.
 *
...
export class RelayQueueService {
   ...
   *
   * platform mode: creates a new FIFO queue in the Derrops account and returns its URL.
   ...
   *                the DerropsSqsPublishRole sqs:SendMessage permission.
   */
  ...
  /**
   * Provision a new FIFO queue in the Derrops account.
    ...
    const res = await this.sqs.send(
      new CreateQueueCommand({
        ...
        Attributes: {
          ...
          // but derrops-cloud already has a 10-minute TTL on jobs so this is
   ...
   * relay-owned queues are never deleted by derrops-cloud.
  ...
  private queueName(tenantId: string, relayId: string): string {
    return `derrops--${tenantId}--relay--middleware--${relayId}.fifo`
  }
}
