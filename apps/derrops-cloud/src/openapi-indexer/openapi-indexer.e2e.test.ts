 ...
 *       config['derrops.oaspec.staging.bucket']
 *       config['derrops.oaspec.storage.bucket']
...
import { config, loadConfig, resetConfigForTests, setConfigForProcess } from '@derrops/config'
import devEnv from '@derrops/config/dev-env'
...
describe('OpenAPI Indexer – staging upload E2E', () => {
  ...
  beforeAll(async () => {
    ...
    await ensureBucketExists(s3Client, config['derrops.oaspec.staging.bucket'])
    await ensureBucketExists(s3Client, config['derrops.oaspec.storage.bucket'])
  }, E2E_TIMEOUT)
  ...
  it(
    ...
    async () => {
      ...
      expect(presignedResult).toMatchObject({
        url: nonEmptyString,
        // key: S3_KEY,
        bucket: config['derrops.oaspec.staging.bucket'],
