...
Derrops Cloud is a NestJS backend API for the Derrops platform. It runs both as a standalone server (development) and as an AWS Lambda function (production).
...
pnpm --filter @derrops/cloud run test -- path/to/file.spec.ts

# Run e2e tests
pnpm --filter @derrops/cloud run test:e2e
...
- **Configuration** – Endpoint, index names, template names, and pipeline ids come from `@derrops/config` (e.g. `opensearch.endpoint`, `opensearch.index.*`, `opensearch.template.*`, `opensearch.pipeline.*`). No hardcoded asset names in opensearch code.
...
**Quick reference**: The committed spec is at `apps/derrops-cloud/src/openapi.json` — read this file to understand the current API surface (endpoints, request/response shapes, DTOs) without running the server.

### Client Generation

Build generates a TypeScript Axios client from OpenAPI spec:

- Output: `../derrops-portal/src/client/derrops-cloud`
- Uses `openapi-generator-cli`
- Runs automatically after spec generation as part of `pnpm --filter @derrops/cloud run build`
...
- OpenSearch: use `@derrops/config` keys (e.g. `opensearch.endpoint`, `opensearch.index.*`, `opensearch.template.*`, `opensearch.pipeline.*`); see `packages/derrops-config`
