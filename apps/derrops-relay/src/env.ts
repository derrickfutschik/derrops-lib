 ...
 * All configuration in derrops-relay comes from process.env — there is no
 * @derrops/config dependency so the relay can be deployed independently.
 */
export const env = {
  ...
  /**
   * API key callers (e.g. derrops-cloud) must supply as `Authorization: Bearer <key>`
   ...
   * When the relay cannot accept inbound connections it polls derrops-cloud
   * for pending jobs and posts results back.
   *
   * RELAY_PLATFORM_URL    — derrops-cloud base URL (e.g. https://api.derrops.com)
   * RELAY_PLATFORM_TOKEN  — Bearer token sent to derrops-cloud (= connection api_key)
   ...
   * `derrops relay start` injects all variables below into process.env.
   *
   ...
   */
  platformSqs: {
    // Accept both the canonical prefixed name and the shorter alias used by derrops-cloud
    ...
    // Initial credentials injected by derrops-cli — refreshed autonomously by the relay
  ...
  cognito: {
    ...
    domain: process.env.RELAY_COGNITO_DOMAIN ?? 'https://auth.derrops.com',
  },

  /**
   * JWT-based authentication for inbound platform requests and Aegis delegation.
   *
   * RELAY_ID                  — UUID assigned by derrops-cloud at registration. Used to validate
   *                             the `aud` claim on inbound platform JWTs.
   * DERROPS_VENDOR_JWKS_URL    — URL of the derrops-cloud JWKS endpoint. Used to validate platform JWTs.
  ...
  jwt: {
    ...
    vendorJwksUrl: process.env.DERROPS_VENDOR_JWKS_URL,
