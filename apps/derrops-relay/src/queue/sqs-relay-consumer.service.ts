 ...
 *
 * When a relay is registered with derrops-cloud as type=local-dev, the platform
 * provisions a dedicated SQS queue for that relay. derrops-cloud publishes jobs
 * to the queue; the relay consumes via SQS long-poll, executes them via
 * ProxyService, and posts results back to derrops-cloud over HTTP.
 *
 ...
 *   Both HAR-wrapped CloudProxyRequestDto and the simpler derrops-cloud wire
...
export class SqsRelayConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    if (hasCognito) {
      // Seed with initial credentials injected by `derrops relay start`
      if (
  ...
  /**
   * Normalise the derrops-cloud wire format to a CloudProxyRequestDto.
   *
   * derrops-cloud sends:
   ...
   * the simpler derrops-cloud wire format, and normalise accordingly.
   */
