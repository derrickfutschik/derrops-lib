...
import { Activity, CheckCircle, Loader2, Pencil, Plus, Trash2, XCircle } from 'lucide-react'
...
export function ConnectionsTab({ connections, aegisInstances, isLoading }: ConnectionsTabProps) {
  ...
  const handleTest = async (conn: CloudRelayConnection) => {
    ...
    try {
      const isHttp =
        conn.delivery_mode === 'direct' ||
        conn.delivery_mode === 'relay-queue' ||
        conn.delivery_mode === 'hybrid'
      const isSqs = conn.delivery_mode === 'platform-queue' || conn.delivery_mode === 'hybrid'

      if (isSqs && !conn.sqs_queue_url) {
        setTestStates((s) => ({
          ...s,
          [conn.id]: { id: conn.id, status: 'err', msg: 'No SQS queue configured' },
        }))
        return
      }
      ...
      if (isSqs && !isHttp) {
        ...
        setTestStates((s) => ({
          ...
          [conn.id]: {
            id: conn.id,
            status: r.sent ? 'ok' : 'err',
            msg: r.sent ? 'Message sent' : (r.error ?? 'Send failed'),
          },
        }))
      } else {
        ...
        setTestStates((s) => ({
          ...
          [conn.id]: {
            id: conn.id,
            status: r.reachable ? 'ok' : 'err',
            msg: r.reachable ? `Reachable — ${r.latencyMs} ms` : (r.error ?? 'Unreachable'),
          },
        }))
      }
    } catch (err: unknown) {
        ...
        <Table>
          ...
          <TableBody>
            {connections.map((conn) => {
              ...
              const aegisName = (conn as CloudRelayConnection & { aegis_id?: string | null })
                .aegis_id
                ? (aegisMap[(conn as CloudRelayConnection & { aegis_id?: string | null }).aegis_id!]
                    ?.name ?? 'Unknown')
                : 'None'
              return (
                      ...
                      {ts && ts.status !== 'loading' && (
                        <span
                          className={cn(
                            ...
                            ts.status === 'ok' ? 'text-success' : 'text-destructive',
                          )}
                        >
                          {ts.status === 'ok' ? (
