...
export function CreateConnectionWizard({ open, onOpenChange }: CreateConnectionWizardProps) {
  ...
  const [sqsOwnership, setSqsOwnership] = useState<SqsOwnership>('derrops')
  ...
  const canAdvance = (): boolean => {
    ...
    if (currentStep === 'sqs')
      return sqsOwnership === 'derrops' || isValidByoQueueUrl(customQueueUrl)
          ...
          {currentStep === 'review' && (
            <div className="space-y-3 text-sm">
              {[
                ...
                connectivity !== 'direct-http'
                  ? ['SQS queue', sqsOwnership === 'derrops' ? 'Derrops-managed' : customQueueUrl]
                  : null,
