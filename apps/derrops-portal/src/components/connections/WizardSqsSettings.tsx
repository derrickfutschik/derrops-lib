...
export type SqsOwnership = 'derrops' | 'custom'
...
const DERROPS_ACCOUNT_ID = '123456789012' // populated at runtime in production
  ...
  return JSON.stringify(
    {
      ...
      Effect: 'Allow',
      Principal: { AWS: `arn:aws:iam::${DERROPS_ACCOUNT_ID}:role/DerropsPlatformRole` },
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {(['derrops', 'custom'] as const).map((mode) => (
            <button
              key={mode}
              type="button"
            ...
            >
              <div className="font-medium text-sm text-foreground mb-1">
                {mode === 'derrops' ? 'Derrops manages' : 'Bring your own'}
              </div>
              <p className="text-xs text-muted-foreground">
                {mode === 'derrops'
                  ? 'Derrops creates and owns the FIFO queue in its AWS account.'
        ...
        <div className="space-y-3">
          <div className="rounded-lg bg-muted/50 border p-3 text-sm text-muted-foreground">
            ...
            <p className="text-xs font-mono">
              derrops--t-&#123;tenant-id&#125;--relay--cloud-relay--&#123;conn-id&#125;.fifo
            </p>
          </div>
