...
export interface CreateCloudRelayConnectionDto {
    /**
     * managed     — Derrops-hosted Lambda relay. self-hosted — Customer-deployed relay on their own infrastructure. local-dev   — Developer local machine. delivery_mode is locked to platform-queue and an SQS FIFO queue is provisioned automatically.
     */
    ...
    'url'?: string;
    /**
     * direct          — derrops-cloud calls relay synchronously. relay-queue     — derrops-cloud submits to relay queue, polls relay for result. platform-queue  — relay polls derrops-cloud outbound. Use when relay cannot accept inbound connections. hybrid          — platform tries direct HTTP first, falls back to SQS on failure. Requires both url and sqs_queue_url. For local-dev relays this is always platform-queue regardless of what is sent.
     */
    'delivery_mode'?: CreateCloudRelayConnectionDtoDeliveryModeEnum;
    /**
     * platform — Derrops provisions and owns the SQS FIFO queue. relay    — Customer provisions the queue and grants sqs:SendMessage to the Derrops platform role; provide relay_sqs_queue_url. Required when delivery_mode is platform-queue or hybrid.
     */
    'sqs_queue_mode'?: CreateCloudRelayConnectionDtoSqsQueueModeEnum;
    /**
     * Customer-owned SQS FIFO queue URL. Required when sqs_queue_mode=relay. The customer must grant the DerropsSqsPublishRole sqs:SendMessage permission on this queue.
     */
