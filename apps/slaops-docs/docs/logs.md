...
Client[Http Client] --> |HTTP Request| SLAOpts[SLAOpsClientListener]  --> |Hook| Har[HAR 1.2 File]
Har[HAR 1.2 File] --> |Enrichment Platform|SLAOpsEnrichedLog[SLAOpsEnrichedLog]
```
...
Bucket convention is `slaops-logs-{data}-{region}-{customer}-{id}`

Clients will be logged into the `slaops-logs-raw-{region}-{customer}-{id}` S3 bucket.
...
Then when logs are enriched, they will be logged to opensearch at index: `slaops-logs-enriched-{region}-{customer}-{id}`.
...
If the request is logged, it will be logged at: `slaops-{region}-logs-har-{customer}-{id}` S3 bucket.
...
If the request is logged, it will be logged at: `slaops-{region}-{customer}-{id}` S3 bucket.
