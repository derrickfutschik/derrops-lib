...
graph TD
    ...
    Response --> Capture
    Capture --> SLAOps[SLAOps Platform]
```
...
sequenceDiagram
    ...
    Interceptor->>SLAOps: send log event
...
```
@slaops/private → @slaops/public → @slaops/client → specific clients
```
...
**Minimal dependencies**: Only `@slaops/private` and `@slaops/client`

### Example 2: Core Types Package

The `@slaops/private` package demonstrates:
