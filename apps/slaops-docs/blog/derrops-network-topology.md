...
All three layers are required for multi-tenant SaaS. Network reachability without IAM conditions is like an unlocked door — traffic can flow but the resource still needs to authorize the caller. See [Tag-Based Tenant Isolation (ABAC)](./derrops-conventions#tag-based-tenant-isolation-abac) for the IAM layer.
