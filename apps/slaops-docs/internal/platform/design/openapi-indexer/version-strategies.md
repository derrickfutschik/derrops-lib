...
Tenant api row (management_mode: 'platform')
  ├── oaSpec.globalOpensearchId   (spec_global_opensearch_id)  → points into slaops--t-glbl0000--oaspec--spec
...
→ searches slaops--t-glbl0000--oaspec--spec
...
4. If the version is new, write the spec file to the tenant's OASpec bucket (`{region}--{env}--slaops--{tenantId}--oaspec--storage--specs`) under the standard key `APIs/{provider}/{service}/{version}/openapi.yaml`.
