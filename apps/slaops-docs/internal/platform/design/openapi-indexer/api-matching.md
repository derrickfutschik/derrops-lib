...
```json
GET /slaops--{tenantId}--oaspec--server--search/_search
{
  "query": {
    "bool": {
      "filter": [
...
The query uses the **Global Search** alias so platform-managed servers (in `slaops--t-glbl0000--oaspec--server`) are resolved alongside tenant-private servers. See [OpenAPI Index Access Pattern — Search Modes](./openapi-index-access-pattern#search-modes).
...
- [OpenSearch Indices — Server Index](./opensearch-indices#2-server-index-slaops--tenantid--oaspec--server) — `OaServerDocument` schema
- [OpenSearch Indices — Operation Index](./opensearch-indices#3-operation-index-slaops--tenantid--oaspec--operation) — `OaOperationDocument` schema and `pathKey`
