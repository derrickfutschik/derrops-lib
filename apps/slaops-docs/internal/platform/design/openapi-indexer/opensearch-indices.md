...
## 1. Spec Index (`slaops--{tenantId}--oaspec--spec`)
...
## 2. Server Index (`slaops--{tenantId}--oaspec--server`)
...
## 3. Operation Index (`slaops--{tenantId}--oaspec--operation`)
...
## 4. Parameter Index (`slaops--{tenantId}--oaspec--param`)
...
## 5. Model Index (`slaops--{tenantId}--oaspec--model`)
...
/** Reserved global tenant ID for the SLAOps-managed public catalogue */
...
// alias: slaops--{tenantId}--oaspec--spec--search
function oaspecAlias(tenantId: string): string {
  return `slaops--${tenantId}--oaspec--spec--search`
}
```
