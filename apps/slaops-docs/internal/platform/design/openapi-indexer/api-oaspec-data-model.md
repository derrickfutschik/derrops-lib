...
Api (PostgreSQL)
 └── OaSpecRef (embedded) → latest_opensearch_id
                            ↓
                        slaops--{tenantId}--oaspec--spec  (OpenSearch — all versions)
...
The SLAOps-managed public catalogue uses the reserved global tenant `t-glbl0000` — the same `varchar(10)` format as all tenant IDs. See [OpenAPI Index Access Pattern](./openapi-index-access-pattern) for how this maps to OpenSearch index scoping.
