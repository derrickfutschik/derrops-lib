...
> **Audience**: Platform engineers working on SLAOps infrastructure and backend services.
> For the customer-facing overview see [Multi-Tenancy (Docs)](/docs/multi-tenancy).

This document defines the full per-tenant resource model, IaC provisioning pattern, access control layering, and naming conventions used to isolate tenants within the SLAOps platform.

Related design documents:
- [Tagging Conventions](./tagging-conventions) — required AWS tags and CDK enforcement
...
SLAOps uses the **pool model** at the infrastructure layer: shared resources (VPC, Aurora cluster, OpenSearch Serverless collection, API Gateway) serve all tenants. Per-tenant isolation is enforced through:
...
The **`slaops`** tenant ID is reserved for platform-owned resources (e.g., the managed OASpec catalogue). It is not generated; it is a hard-coded constant in `@slaops/config`:

```typescript
config['opensearch.oaspec.slaops-tenant-id'] // = "slaops"
```
...
|---|---|---|
| OASpec storage | `{region}--{env}--slaops--{tenantId}--oaspec--storage--specs` | `us-east-1--prod--slaops--t-a3f8b2--oaspec--storage--specs` |
| Log archive | `{region}--{env}--slaops--{tenantId}--logging--storage--logs` | `us-east-1--prod--slaops--t-a3f8b2--logging--storage--logs` |

See [OASpec Bucket (Docs)](/docs/oaspec-bucket) for the full bucket naming reference including the global SLAOps-managed bucket.
...
| Search alias | `oaspec-search-{tenantId}` | Created at onboarding, pointing to `oaspec-slaops` only; updated on first private upload to include the private index |
...
| Resource | Pattern | Example |
|---|---|---|
| Tenant execution role | `slaops--platform--api--{tenantId}--role` | `slaops--platform--api--t-a3f8b2--role` |
...
- OpenSearch `aoss:ReadDocument` on `oaspec-slaops` index
...
`packages/slaops-infra/lib/constructs/tenant-construct.ts`

Provisions the complete per-tenant resource set from a single `tenantId` input. Tags every child resource via `Tags.of(this)` per [Tagging Conventions](./tagging-conventions).
...
|---|---|---|
| Onboarding | `TenantConstruct` deployed | Both S3 buckets, IAM role, and `oaspec-search-{tenantId}` alias created (pointing to `oaspec-slaops` only) |
...
| **AWS IAM** | Per-tenant scoped role; resource-level policies on S3 buckets and OpenSearch indices; `aws:ResourceTag/slaops:tenant-id` IAM condition | CDK TenantConstruct |
...
## Naming Reference (SLAOps-specific)

Full resource naming summary applying the [Derrops compound kebab-case convention](/blog/derrops-naming-sheet):

| Resource | Pattern | Example |
|---|---|---|
| OASpec S3 bucket | `{region}--{env}--slaops--{tenantId}--oaspec--storage--specs` | `us-east-1--prod--slaops--t-a3f8b2--oaspec--storage--specs` |
| Logs S3 bucket | `{region}--{env}--slaops--{tenantId}--logging--storage--logs` | `us-east-1--prod--slaops--t-a3f8b2--logging--storage--logs` |
| OpenSearch index | `oaspec-{tenantId}` | `oaspec-t-a3f8b2` |
| OpenSearch alias | `oaspec-search-{tenantId}` | `oaspec-search-t-a3f8b2` |
| IAM role | `slaops--platform--api--{tenantId}--role` | `slaops--platform--api--t-a3f8b2--role` |
| Aurora schema | `t_{tenantId_underscored}` | `t_a3f8b2` |
| DynamoDB partition prefix | `{tenantId}:` | `t-a3f8b2:` |
| CloudFormation stack | `slaops--platform--tenant--{tenantId}--stack` | `slaops--platform--tenant--t-a3f8b2--stack` |
...
- [Tagging Conventions](./tagging-conventions) — required tags, CDK enforcement, cost allocation and security use cases
...
- [OASpec Bucket (Docs)](/docs/oaspec-bucket) — S3 bucket naming reference including the global SLAOps-managed bucket
