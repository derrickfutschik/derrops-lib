...
title: API Tester
sidebar_position: 6
description: Send HTTP requests through a relay from the SLAOps Portal and inspect the results.
tags:
  - quickstart
  - api-tester
  - relay
...
- Confirm the relay process is running (`docker logs slaops-relay` or check the relay status in the Portal).
