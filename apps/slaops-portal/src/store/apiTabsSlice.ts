...
type TabName = keyof Pick<ApiTabsState, 'versions' | 'operations' | 'servers' | 'parameters' | 'models'>

const initialState: ApiTabsState = {
  selectedVersion: null,
  versions: { sort: { field: 'indexedAt', direction: 'desc' }, hiddenColumns: [], page: 0 },
  operations: { sort: { field: 'path', direction: 'asc' }, hiddenColumns: [], page: 0, query: '', methodFilter: [], tagFilter: null },
  servers: { sort: { field: 'serverIndex', direction: 'asc' }, hiddenColumns: [], page: 0 },
  parameters: { sort: { field: 'name', direction: 'asc' }, hiddenColumns: [], page: 0, query: '', locationFilter: null },
  models: { sort: { field: 'name', direction: 'asc' }, hiddenColumns: [], page: 0, query: '', usedInFilter: null },
}
...
const apiTabsSlice = createSlice({
  ...
  reducers: {
    ...
    setTabSort(state, action: PayloadAction<{ tab: TabName; field: string; direction: 'asc' | 'desc' }>) {
...
export const {
  ...
  setSelectedVersion: { description: 'Pins a specific spec version for all detail tabs; null reverts to latest.', area: ActionArea.UI, group: ActionGroup.Apis },
  resetAllTabs: { description: 'Resets all tab state to defaults — called when navigating to a different API.', area: ActionArea.UI, group: ActionGroup.Apis },
  setTabSort: { description: 'Sets the sort field and direction for a specific detail tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  setTabPage: { description: 'Sets the current page number for a specific detail tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  toggleTabColumn: { description: 'Toggles visibility of a column in a specific detail tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  showAllTabColumns: { description: 'Restores all hidden columns in a specific detail tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  setOperationsQuery: { description: 'Sets the free-text search query for the operations tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  setOperationsMethodFilter: { description: 'Sets the HTTP method filter for the operations tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  setOperationsTagFilter: { description: 'Sets the tag filter for the operations tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  setParametersQuery: { description: 'Sets the free-text search query for the parameters tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  setParametersLocationFilter: { description: 'Sets the parameter location filter (path/query/header/cookie).', area: ActionArea.UI, group: ActionGroup.Apis },
  setModelsQuery: { description: 'Sets the free-text search query for the models tab.', area: ActionArea.UI, group: ActionGroup.Apis },
  setModelsUsedInFilter: { description: 'Sets the "used in" filter (request/response) for the models tab.', area: ActionArea.UI, group: ActionGroup.Apis },
})
