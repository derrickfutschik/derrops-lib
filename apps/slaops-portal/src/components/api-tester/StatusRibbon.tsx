...
export function StatusRibbon({
  ...
  return (
    <div
      className={`flex items-center justify-between ${className} border-t border-border bg-muted/30 text-xs text-muted-foreground`}
    >
          ...
          <>
            {jsonStats?.type === 'array' && (
              <span>
                ...
                {jsonStats.flattenedCount !== jsonStats.count &&
                  ` (${jsonStats.flattenedCount.toLocaleString()} flattened)`}
              </span>
            )}
            {jsonStats?.type === 'array' && <span>depth {jsonStats.depth}</span>}
            {duplicateCount > 0 && (
              <span className="text-red-400">{duplicateCount} duplicates</span>
            )}
            {jsonStats && jsonStats.totalKeys > 0 && (
              <span>{jsonStats.totalKeys.toLocaleString()} total keys</span>
            )}
            ...
            {filterPercent !== null && (
              <span className="text-primary">{filterPercent}% of response</span>
            )}
          </>
        )}
        {viewMode === 'markdown' && (
