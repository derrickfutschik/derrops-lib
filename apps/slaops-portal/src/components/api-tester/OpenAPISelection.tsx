...
export function OpenAPISelection({
  ...
  useEffect(() => {
    ...
    const pathLevelParams = (Array.isArray(pathLevelParamsRaw) ? pathLevelParamsRaw : []).map(
      (p: any) => resolveRef(spec, p),
    )
    const operationLevelParams = (
      Array.isArray(operationLevelParamsRaw) ? operationLevelParamsRaw : []
    ).map((p: any) => resolveRef(spec, p))
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <label
          className={`text-sm font-medium ${selectedServiceId ? 'text-muted-foreground' : 'text-destructive'}`}
        >
          API Service
        </label>
        ...
        <>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) => onOperationChange(v || null)}
            >
              <SelectTrigger className="bg-background">
