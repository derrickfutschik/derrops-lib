                ...
                {isUnspecified ? (
                  <Popover>
                    ...
                    <PopoverContent className="p-2">
                      <p className="text-sm">This parameter is not defined in the OpenAPI specification</p>
                    </PopoverContent>
                  </Popover>
                ) : (
                  <span className={`font-mono font-medium truncate ${!isValid ? 'text-destructive' : ''}`}>{name}</span>
                )}
                <Badge variant="outline" className={`text-xs flex-shrink-0 ${isUnspecified ? 'text-orange-500 border-orange-500/50' : ''}`}>
                  {isUnspecified ? 'unknown' : type}
                </Badge>
                {isUnspecified ? (
                  <Badge variant="outline" className="text-xs text-orange-500 border-orange-500/50 flex-shrink-0">
                    Unspecified
                  </Badge>
                ) : required ? (
                  <Badge 
            ...
            <div className="mt-1 text-muted-foreground flex items-center gap-1">
              ...
              {displayValue ? (
                isUsingDefault ? (
                  <Popover>
                    <PopoverTrigger asChild>
                      <span className="text-muted-foreground/60 italic cursor-help truncate">{displayValue}</span>
                    </PopoverTrigger>
                    <PopoverContent className="p-2">
                      <p className="text-sm">Default value from OpenAPI spec (not explicitly provided)</p>
                    </PopoverContent>
                  </Popover>
                ) : (
                  <span className={`truncate ${isUnspecified ? 'text-orange-500' : isValid ? 'text-green-500' : 'text-destructive'}`}>
                    {displayValue}
                  </span>
                )
              ) : (
            ...
            {!isUnspecified && (
              <div className="mt-1 flex items-center gap-1">
                ...
                <Popover>
                  ...
                  <PopoverContent className="p-2">
                    <p className="text-sm">{validationReason || (isValid ? "Parameter is valid" : "Parameter is invalid")}</p>
                  </PopoverContent>
                </Popover>
              </div>
            )}
            {isUnspecified && (
              <div className="mt-1 flex items-center gap-1">
                ...
                <Popover>
                  ...
                  <PopoverContent className="p-2">
                    <p className="text-sm">Parameter not in specification - may be ignored by the API</p>
                  </PopoverContent>
                </Popover>
              </div>
            )}
            ...
            <div className="mt-3 pt-3 border-t border-border">
              <Tabs defaultValue={defaultTab} className="w-full">
                <TabsList className="h-7 w-full justify-start">
                  <TabsTrigger value="description" className="text-xs h-5 px-2" disabled={!description}>
                    Description
                  </TabsTrigger>
