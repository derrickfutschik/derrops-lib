...
function highlightJsonWithErrors(content: string, error: JsonError | null): string {
  ...
  const highlightedLines = lines.map((line, index) => {
    ...
    const patterns = [
      { regex: /("(?:[^"\\]|\\.)*")(\s*:)/g, replacement: '<span class="text-purple-400">$1</span>$2' },
      { regex: /:\s*("(?:[^"\\]|\\.)*")/g, replacement: ': <span class="text-green-400">$1</span>' },
...
export function RequestBodyEditor({
  ...
  const updateFormDataEntry = (index: number, field: keyof FormDataEntry, newValue: string | boolean) => {
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex items-center space-x-2">
          ...
          <Label htmlFor="body-form-data" className="text-sm cursor-pointer">form-data</Label>
        </div>
        <div className="flex items-center space-x-2">
          <RadioGroupItem value="x-www-form-urlencoded" id="body-urlencoded" />
          <Label htmlFor="body-urlencoded" className="text-sm cursor-pointer">x-www-form-urlencoded</Label>
        </div>
        <div className="flex items-center space-x-2">
          <RadioGroupItem value="raw" id="body-raw" />
          <Label htmlFor="body-raw" className="text-sm cursor-pointer">raw</Label>
        </div>
        <div className="flex items-center space-x-2">
          <RadioGroupItem value="binary" id="body-binary" />
          <Label htmlFor="body-binary" className="text-sm cursor-pointer">binary</Label>
        </div>
      </RadioGroup>
...
export type { BodyType, RawType, FormDataEntry };
