...
export class CloudRelayController {
  ...
  @ApiOperation({
    summary: 'Enqueue an async proxy job (called by slaops-cloud)',
    description:
      'slaops-cloud submits a job here instead of waiting for a direct proxy response. ' +
  ...
  @ApiHeader({ name: 'authorization', required: true, description: 'Bearer <platform-jwt>' })
  @ApiOperation({
    summary: 'Get the status and result of a queued job (called by slaops-cloud)',
  })
