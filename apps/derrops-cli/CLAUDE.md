...
oclif-based CLI (`derrops`) that lets developers register and run a local relay, routing Derrops Portal requests to `localhost` services via SQS. Published as `derrops-cli` on npm.
...
│   ├── commands/
│   │   └── relay/
│   │       ├── init.ts           # derrops relay init
│   │       └── start.ts          # derrops relay start
│   ├── auth/
│   │   └── cognito.ts            # PKCE browser auth, token refresh, Identity Pool creds
│   ├── config-file.ts            # ~/.derrops/config reader/writer (0644)
│   ├── credentials-file.ts       # ~/.derrops/credentials reader/writer (0600)
...
Authenticates via Cognito browser PKCE, registers a `local-dev` relay with `derrops-cloud`, and persists configuration.
...
5. Writes non-sensitive config to `~/.derrops/config` (mode 0644).
6. Writes Cognito tokens to `~/.derrops/credentials` (mode 0600).

No AWS credentials are stored — they are obtained at relay start via the Identity Pool and held in memory only.

### `derrops relay start`

Exchanges stored Cognito tokens for temporary AWS credentials, then delegates to `derrops-relay`'s `bootstrapRelay()`.
...
1. Reads `~/.derrops/config` and `~/.derrops/credentials` for the given profile.
2. If the Cognito access token is expired (within 60s buffer), calls `refreshAccessToken()` and writes the refreshed tokens back to `~/.derrops/credentials`.
...
5. Calls `bootstrapRelay()` from the `derrops-relay` workspace package (loaded via `require` to avoid circular import issues).
...
| `DERROPS_IDENTITY_POOL_ID`     | _(must be set in release builds)_ |
...
pnpm --filter derrops-cli run build

# Watch mode
pnpm --filter derrops-cli run dev
...
The CLI depends on `derrops-relay` (workspace package) — build it first if relay changes are in flight:

```bash
pnpm --filter derrops-relay run build
pnpm --filter derrops-cli run build
```
...
- **No AWS SDK** in the CLI itself — `getAwsCredentialsFromIdentityPool` uses `fetch` directly to avoid the package weight. The `@aws-sdk/client-sqs` dependency is `optionalDependencies` and is only used inside `derrops-relay`.
- **No `process.env` for config reads** — Cognito constants are resolved once at module load from `process.env` with compile-time defaults. The `@derrops/config` package is not used here (CLI has no CDK/Amplify config context).
...
- User-facing local relay setup: `apps/derrops-docs/public/docs/quickstart/local-relay.md`
- Cloud relay (Docker / Lambda): `apps/derrops-docs/public/docs/quickstart/cloud-relay.md`
- Relay runtime: `apps/derrops-cloud/src/cloud-relay/` (NestJS service that the relay polls)
