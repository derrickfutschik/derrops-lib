...
export default class RelayStart extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (!config?.relay_id) {
      this.error(
        `No relay config found for profile [${profileName}] in ${CONFIG_FILE}.\n` +
          `Run 'derrops relay init${profileName !== 'default' ? ` --profile ${profileName}` : ''}' first.`,
      )
    }
    ...
    for (const key of [
      ...
      if (!config[key]) {
        this.error(
          `Missing '${key}' in config profile [${profileName}].\nRun 'derrops relay init --force' to re-register.`,
        )
      }
    }
      ...
      try {
        ...
        this.error(
          `Token refresh failed: ${(err as Error).message}\n` +
            `Your 30-day session has likely expired. Run 'derrops relay init --force' to re-authenticate.`,
        )
      }
    }
    ...
    // Config values come from ~/.derrops/config; tokens from ~/.derrops/credentials.
    ...
    process.env.RELAY_COGNITO_CLIENT_ID = process.env.DERROPS_COGNITO_CLIENT_ID ?? ''
    process.env.RELAY_COGNITO_DOMAIN =
      process.env.DERROPS_COGNITO_DOMAIN ?? 'https://auth.derrops.com'
    ...
    const { bootstrapRelay } = require('derrops-relay') as { bootstrapRelay: () => Promise<void> }
