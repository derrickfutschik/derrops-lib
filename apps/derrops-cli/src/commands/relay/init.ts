...
const DEFAULT_PLATFORM_URL = 'https://api.derrops.com'
...
async function registerRelay(
  ...
  // Send the access_token as Bearer. derrops-cloud verifies it against Cognito
...
export default class RelayInit extends Command {
  static description =
    'Authenticate with Derrops via browser and register a local-dev relay. Re-run every 30 days when the refresh token expires.'

  static examples = [
    '<%= config.bin %> relay init',
    '<%= config.bin %> relay init --profile staging --platform-url https://api.staging.derrops.com',
  ...
  static flags = {
    'platform-url': Flags.string({
      description: 'Derrops platform base URL (prompted if omitted)',
    }),
    profile: Flags.string({
      char: 'p',
      description:
        'Profile name in ~/.derrops/config and ~/.derrops/credentials (default: "default")',
      default: 'default',
    }),
    force: Flags.boolean({
      char: 'f',
    ...
    'queue-mode': Flags.string({
      ...
      default: 'platform',
      description:
        'platform (default) — Derrops provisions and owns the SQS FIFO queue.\n' +
        'relay              — You provision the queue in your own AWS account and grant the DerropsSqsPublishRole sqs:SendMessage permission.',
    }),
  ...
  async run(): Promise<void> {
    ...
    if (existing?.relay_id && !flags.force) {
      ...
      this.log(`  derrops relay start${profile !== 'default' ? ` --profile ${profile}` : ''}`)
      return
    }
    ...
    // Register relay — derrops-cloud verifies the access_token, reads tenantId
    ...
    if (sqsQueueMode === 'relay') {
      ...
      this.log(`  Ensure the DerropsSqsPublishRole has sqs:SendMessage on:`)
      this.log(`    ${registration.sqs_queue_url}`)
    }

    // Write non-sensitive relay configuration to ~/.derrops/config (0644).
    ...
    setConfigProfile(
      {
        ...
        identity_pool_id: process.env.DERROPS_IDENTITY_POOL_ID ?? IDENTITY_POOL_ID,
        cognito_region: process.env.DERROPS_COGNITO_REGION ?? COGNITO_REGION,
        user_pool_id: process.env.DERROPS_COGNITO_USER_POOL_ID ?? USER_POOL_ID,
      },
      profile,
    )

    // Write sensitive Cognito tokens to ~/.derrops/credentials (0600).
    ...
    this.log(
      `\n  Run 'derrops relay start${profile !== 'default' ? ` --profile ${profile}` : ''}' to connect.`,
    )
