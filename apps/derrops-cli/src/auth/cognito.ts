 ...
 *
 * DERROPS_COGNITO_DOMAIN       — Cognito hosted UI base URL (no trailing slash)
 * DERROPS_COGNITO_CLIENT_ID    — OAuth 2.0 public client ID (PKCE — no client secret)
 * DERROPS_COGNITO_REGION       — AWS region for the User Pool (default: ap-southeast-2)
 * DERROPS_COGNITO_USER_POOL_ID — Cognito User Pool ID (used as Identity Pool provider key)
 * DERROPS_IDENTITY_POOL_ID     — Cognito Identity Pool ID (for AWS credential exchange)
 */
export const COGNITO_DOMAIN = process.env.DERROPS_COGNITO_DOMAIN ?? 'https://auth.derrops.com'
export const CLIENT_ID = process.env.DERROPS_COGNITO_CLIENT_ID ?? ''
export const COGNITO_REGION = process.env.DERROPS_COGNITO_REGION ?? 'ap-southeast-2'
export const USER_POOL_ID = process.env.DERROPS_COGNITO_USER_POOL_ID ?? ''
export const IDENTITY_POOL_ID = process.env.DERROPS_IDENTITY_POOL_ID ?? ''
...
export async function getAwsCredentialsFromIdentityPool(
  ...
  if (!poolId) throw new Error('DERROPS_IDENTITY_POOL_ID is not configured.')
  if (!userPoolId) throw new Error('DERROPS_COGNITO_USER_POOL_ID is not configured.')
...
export async function authenticateWithBrowser(): Promise<CognitoTokens> {
  if (!CLIENT_ID) {
    throw new Error(
      'DERROPS_COGNITO_CLIENT_ID is not configured. ' +
        'Set the environment variable or use a release build of derrops-cli.',
    )
  }
