...
export class OpenApiParserService {
  ...
  validateOpenApi3(spec: unknown, s3Key: string): asserts spec is OpenApiSpec {
    ...
    if (!s || typeof s !== 'object') {
      throw new IndexingError(
        ...
        IndexingErrorCode.VALIDATION_ERROR,
        s3Key,
      )
    }
    ...
    if (!s.info || typeof s.info !== 'object' || !s.info.title) {
      throw new IndexingError(
        'Invalid OpenAPI spec: missing info.title',
        IndexingErrorCode.VALIDATION_ERROR,
        s3Key,
      )
    }
  ...
  transformToIndexDocument(
    ...
    const sampleOperations = allOperations
      .slice(0, INDEXING_LIMITS.MAX_SAMPLE_OPERATIONS)
      .map((op) => ({
        method: op.method,
        path: op.path,
    ...
    const operationSearchText = searchTextParts
      .join(' ')
      .slice(0, INDEXING_LIMITS.MAX_SEARCH_TEXT_LENGTH)
