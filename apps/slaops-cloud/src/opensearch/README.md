...
This module **owns OpenSearch assets and settings** for the slaops-cloud app: it provides the AWS OpenSearch Serverless client and manages index templates and ingest pipelines according to the project convention.

## Convention

- **Configuration** – All endpoint, index names, template names, and pipeline ids come from `@slaops/config` (e.g. `opensearch.endpoint`, `opensearch.index.*`, `opensearch.template.*`, `opensearch.pipeline.*`). No hardcoded asset names.
