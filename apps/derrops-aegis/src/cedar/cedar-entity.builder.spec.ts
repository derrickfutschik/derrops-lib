...
describe('buildEntities', () => {
  ...
  it('marks federated users as isFederated=true with correct idpProvider', () => {
    const token: CognitoTokenPayload = {
      ...
      identities: JSON.stringify([
        { providerName: 'OktaSSO', providerType: 'SAML', primary: true },
      ]),
    }
  ...
  it('creates UserGroup entities for each cognito:groups entry', () => {
    ...
    const groups = entities.filter(
      (e) => (e.uid as { type: string }).type === 'AegisNamespace::UserGroup',
    )
  ...
  it('creates ApiEnvironment, ApiHost, and ApiEndpoint entities', () => {
    ...
    const env = entities.find(
      (e) => (e.uid as { type: string }).type === 'AegisNamespace::ApiEnvironment',
    )
    expect((env!.uid as { id: string }).id).toBe('prod')

    const host = entities.find(
      (e) => (e.uid as { type: string }).type === 'AegisNamespace::ApiHost',
    )
    ...
    const endpoint = entities.find(
      (e) => (e.uid as { type: string }).type === 'AegisNamespace::ApiEndpoint',
    )
    expect((endpoint!.uid as { id: string }).id).toBe('getOrderById')
    expect(endpoint!.parents).toEqual([
      { type: 'AegisNamespace::ApiHost', id: 'payments.internal' },
    ])
  })
