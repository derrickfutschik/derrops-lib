...
/**
 * Completes the Aegis registration handshake with the Derrops platform at startup.
 *
 * If DERROPS_REGISTRATION_TOKEN and DERROPS_PLATFORM_URL are both set, this service
...
export class RegistrationService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (!registrationToken || !platformUrl) {
      this.logger.log(
        'DERROPS_REGISTRATION_TOKEN or DERROPS_PLATFORM_URL not set — skipping registration handshake',
      )
      return
    }
  ...
  private async performHandshake(platformUrl: string, registrationToken: string): Promise<void> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.logger.log(
          'Remove DERROPS_REGISTRATION_TOKEN from the environment (token is now consumed)',
        )
      } else {
