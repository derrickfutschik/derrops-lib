...
import React, { useMemo, useRef, useState, useCallback, useEffect, Component } from 'react'
...
export function MaximizableCodeViewer({
  ...
  const tableData = useMemo(() => {
    const src = displayContent
    try {
      const parsed = JSON.parse(src)
      if (Array.isArray(parsed)) {
        if (parsed.length === 0) return null
        if (typeof parsed[0] === 'object' && parsed[0] !== null && !Array.isArray(parsed[0])) {
          const allKeys = Array.from(new Set(parsed.flatMap((item: any) => Object.keys(item))))
          return {
            columns: allKeys,
            rows: parsed.map((item: any) => allKeys.map((k) => item[k] !== undefined ? String(item[k]) : '')),
          }
        }
        return {
          columns: ['Value'],
          rows: parsed.map((item: any) => [String(item)]),
        }
      }
      return null
    } catch {
  ...
  const renderMarkdownView = () => {
    ...
    const fallback = (
      <div className="p-4 text-sm text-destructive">
        Failed to render Markdown. Switch to JSON view to see the data.
      </div>
    )
    return (
      <MarkdownErrorBoundary key={mdContent} fallback={fallback}>
        <div className="p-4 prose prose-sm dark:prose-invert max-w-none prose-headings:text-foreground prose-p:text-foreground prose-li:text-foreground prose-strong:text-foreground prose-a:text-primary prose-code:text-foreground prose-pre:bg-muted prose-pre:text-foreground prose-blockquote:text-muted-foreground prose-blockquote:border-border">
  ...
  const renderTableView = () => {
    if (!tableData) {
      return (
        <div className="p-4 text-sm text-muted-foreground">
          Content cannot be displayed as a table. Switch to JSON view to see the data.
        </div>
      )
    }
    return (
  ...
  const viewModeTabs = (
    <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as ViewMode)} className="h-auto">
      <TabsList className="h-7 p-0.5 gap-0.5">
        <TabsTrigger value="json" className="h-6 px-2 text-xs">JSON</TabsTrigger>
        <TabsTrigger value="markdown" className="h-6 px-2 text-xs">Markdown</TabsTrigger>
        <TabsTrigger value="table" className="h-6 px-2 text-xs">Table</TabsTrigger>
      </TabsList>
    </Tabs>
  )

  const renderActiveView = (unlimitedHeight = false) => {
    if (viewMode === 'markdown') return renderMarkdownView()
    if (viewMode === 'table') return renderTableView()
    return (
      <pre ref={unlimitedHeight ? dialogPreRef : normalPreRef} className="text-sm font-mono text-foreground whitespace-pre-wrap break-all">
        <code>{renderContent(unlimitedHeight)}</code>
      </pre>
    )
  }
