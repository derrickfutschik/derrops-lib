/**
 * OpenAPI Parser - Parses OpenAPI 3.x specs and transforms them for indexing
 */

import * as yaml from 'yaml';
import { OpenApiIndexDocument, IndexingError, IndexingErrorCode, INDEXING_LIMITS } from './types';
 ...
 */
export function detectFormat(s3Key: string): 'yaml' | 'json' {
 ...
 */
export function parseContent(content: string, format: 'yaml' | 'json'): unknown {
  try {
    if (format === 'json') {
 ...
 */
export function extractPathComponents(s3Key: string): { provider: string; service: string; version: string } {
 ...
 */
export function validateOpenApi3(spec: unknown, s3Key: string): asserts spec is OpenApiSpec {
  const s = spec as any;
 ...
 */
function extractUniqueTags(spec: OpenApiSpec): string[] {
 ...
 */
export function transformToIndexDocument(
  spec: OpenApiSpec,
  s3Key: string,
  bucket: string,
  fileSize: number,
  ...
  const { provider, service, version } = extractPathComponents(s3Key);
...
export function parseAndTransform(
  ...
  const format = detectFormat(s3Key);
  const rawSpec = parseContent(content, format);

  // Validate OpenAPI 3.x
  validateOpenApi3(rawSpec, s3Key);

  // Transform to index document
  return transformToIndexDocument(rawSpec, s3Key, bucket, fileSize, format);
}
