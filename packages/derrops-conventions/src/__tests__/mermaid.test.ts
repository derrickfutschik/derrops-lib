...
describe('toMermaid()', () => {
  describe('shape', () => {
    it('renders a single instance with no children as a leaf node', () => {
      const root = new DerropsConventions({ org: 'derrops', region: 'ap-southeast-2', env: 'dev' })
      const out = root.toMermaid()
      expect(out.split('\n')).toEqual([
        'flowchart TD',
        '  org_derrops["region: ap-southeast-2<br/>env: dev<br/>org: derrops"]',
      ])
    })
    ...
    it('renders a linear org → domain → service chain as nested subgraphs', () => {
      ...
      expect(out).toContain('subgraph org_derrops["org: derrops"]')
  ...
  describe('options', () => {
    it('respects a custom direction', () => {
      const root = new DerropsConventions({ org: 'derrops' })
    ...
    it('shows the ARN context account ID on the root only when enabled', () => {
      const org = new DerropsConventions({ org: 'derrops' }).arnContext({
      ...
      const rootLine = out.split('\n').find((l) => l.includes('subgraph org_derrops'))
  ...
  describe('rule compliance', () => {
    ...
    it('quotes every label with double quotes and contains no semicolons', () => {
      const org = new DerropsConventions({ org: 'derrops', region: 'ap-southeast-2', env: 'dev' })
      org.with({ domain: 'platform' }).with({ service: 'app-database' })
      const out = org.toMermaid()
