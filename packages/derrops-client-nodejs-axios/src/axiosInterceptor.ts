...
import type { HarEntry, HarLog, HarLogListener } from '../../derrops-public/dist'
import { redact } from '../../derrops-public/dist'
import { DerropsClient } from './DerropsClient'
...
export type InterceptorOptions = {
  ...
  buildExtensions?: (
    ...
    res?: AxiosResponse,
  ) => Record<`_${string}`, unknown> | undefined
  listeners?: HarLogListener[]
}

const isInternal = (cfg: InternalAxiosRequestConfig) => cfg.headers?.['x-derrops-internal'] === '1'
...
function extractQueryString(
  ...
  const serializeParams = (): string | undefined => {
    const serializer = (
      cfg as InternalAxiosRequestConfig & {
 ...
 */
export function attachDerropsInterceptor(instance: AxiosInstance, options: InterceptorOptions) {
  ...
  instance.interceptors.response.use(
    async (res) => {
      ...
      const requestBody =
        options.includeRequestBody && cfg.data
      ...
      const responseBody =
        options.includeResponseBody && res.data
          ...
          content: {
            size: Number(
              res.headers?.['content-length'] ??
                (responseBody ? Buffer.byteLength(responseBody, 'utf8') : 0),
            ),
      ...
      const harLog: HarLog = {
        version: '1.2',
        creator: {
          name: 'derrops-client-nodejs-axios',
          version: '1.0.0',
        },
        entries: [entry],
      }
    ...
    async (error) => {
      ...
      const responseBody =
        options.includeResponseBody && error.response?.data
      ...
      const entry: HarEntry = {
        ...
        response: {
          ...
          headers: error.response
            ? headersToHar(redact(error.response.headers, options.redactHeaders))
            : [],
          content: {
            size: Number(
