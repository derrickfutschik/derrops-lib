...
import {
  EnrichedOperationObject,
  EnrichedOperationObjectWithValidation,
  HttpRequest,
  ParamInRequest,
  RelativeRequest,
...
function flatMapOperations<T extends {} = {}>(doc: OpenAPIV3_1.Document<T>) {
  return Object.entries(doc.paths!).flatMap(([path, pathObj]) =>
    Object.entries(pathObj!).flatMap(([method, operation]) => ({
      path,
...
export function findOperation<T extends {} = {}>(
  request: HttpRequest,
  doc: OpenAPIV3_1.Document<T>,
): EnrichedOperationObject {
  const url = new URL(request.url)
  const path =
    doc.servers && doc.servers.length > 0
      ? calculatePath(request.url, doc.servers?.[0]!.url)
...
export function buildRequestObjects(
  ...
  const validationP = validation.errors.find(
    (er) =>
      (er.location.replace('headers', 'header') + '.' + er.path).toLowerCase() ===
      path.toLowerCase(),
  )
...
export function mapErrorToParamRequest(
  err: t.ValidationError,
  request: HttpRequest,
  operation: OpenAPIV3_1.OperationObject,
): RequestParameterObject {
  switch (err.location) {
...
export function enrichOperationWithValidation(
  request: HttpRequest,
  operation: EnrichedOperationObject,
  validation: t.Validation = { status: 200, errors: [] },
  ...
  const parameters: ParamInRequest[] = operation.parameters
    ...
    .map((param) => ({
      param,
      input: findInput(operation.restops.path, param.in, param.name, request),
    }))
    .map(({ param, input }) => ({
      ...param,
      restops: {
        ...
        // TODO
        validation: validation.errors.find(
          (error) => error.location === param.in && error.path == param.name,
        ) ?? { errorCode: 'valid', message: 'valid' },
      },
    }))!
...
export function mapRequestToParamRequest(
  request: HttpRequest,
  operation: OpenAPIV3_1.OperationObject,
  validation: t.Validation = { status: 200, errors: [] },
) {
      ...
      : Object.entries(request.query!).map(([name, value]) =>
          createRequestParam(
            ...
            validation.errors.find(
              (err) => err.location === 'query' && err.path.toLowerCase() === name.toLowerCase(),
            ) ?? validParam(name, 'query'),
            'query',
          ),
        )

  const headerParams = Object.entries(request.headers!).map(([name, value]) =>
    createRequestParam(
      ...
      value, // todo parse if need
      validation.errors.find(
        (err) => err.location === 'headers' && err.path.toLowerCase() === name.toLowerCase(),
      ) ?? validParam(name, 'headers'),
      'headers',
    ),
  )
  ...
  return [...queryParams, ...headerParams, ...bodyValidations]
}

export function openAPI<T extends {} = {}>(
  requestRaw: HttpRequest,
  openApiDoc: OpenAPIV3_1.Document<T>,
) {
  ...
  const paramsIn = (inn: 'headers' | 'query') =>
...
const exactMatch = (
  ...
  operations.find(
    (operation) => operation.method === request.method && operation.path === request.path,
  )

export function matchPath<T extends {} = {}>(
  requestRaw: RelativeRequest,
  doc: OpenAPIV3_1.Document<T>,
) {
  const request = {
    ...requestRaw,
  ...
  return (
    exactMatch(request, operations) ??
    operations.find((operation) => pathsMatch(request, operation)) ??
    operations.find((operation) => pathsMatch(request, operation, true))
  )
}
...
function pathsMatch<T extends {} = {}>(
  ...
  match: {
    path: string
    method: OpenAPIV3_1.HttpMethods
    operation: OpenAPIV3_1.OperationObject<T>
  },
  ...
  const pathParams =
    (match.operation.parameters &&
      match.operation
        .parameters!.filter((po) => (po as OpenAPIV3_1.ParameterObject).in == 'path')
