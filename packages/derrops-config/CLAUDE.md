...
Zod-validated, type-safe configuration management for the Derrops platform. Provides a `config` singleton with dot-notation key access. All application code must use this module — never `process.env` directly.
...
```typescript
import { loadConfig, resetConfigForTests } from '@derrops/config'
...
config['opensearch.oaspec.index']('t-abc123', 'spec') // derrops--dev--t-abc123--oaspec--spec
config['opensearch.oaspec.search-alias']('t-abc123', 'spec') // derrops--dev--t-abc123--oaspec--spec--search

// Legacy indices
config['opensearch.index.openapi.apis'] // derrops-openapi-apis-dev
