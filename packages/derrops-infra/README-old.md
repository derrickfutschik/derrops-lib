...
AWS CDK Infrastructure Stacks for Derrops Platform
...
- **Environment-based Naming**: `${env}.internal.derrops.com`
...
| ------------------------------- | -------------------- |
| `derrops-vpc-id`                | VPC ID               |
| `derrops-vpc-cidr-block`        | VPC CIDR block       |
| `derrops-vpc-subnet-public-a`   | Public subnet A ID   |
| `derrops-vpc-subnet-public-b`   | Public subnet B ID   |
| `derrops-vpc-subnet-public-c`   | Public subnet C ID   |
| `derrops-vpc-subnet-private-a`  | Private subnet A ID  |
| `derrops-vpc-subnet-private-b`  | Private subnet B ID  |
| `derrops-vpc-subnet-private-c`  | Private subnet C ID  |
| `derrops-vpc-subnet-isolated-a` | Isolated subnet A ID |
| `derrops-vpc-subnet-isolated-b` | Isolated subnet B ID |
| `derrops-vpc-subnet-isolated-c` | Isolated subnet C ID |
...
| `DerropsUserPoolArn`          | Cognito User Pool ARN            |
| `DerropsUserPoolClientId`     | User Pool Client ID for web apps |
| `DerropsUserPoolProviderName` | User Pool provider name          |
| `DerropsUserPoolProviderUrl`  | User Pool provider URL           |

### Database Stack Exports

| Export Name                    | Description                        |
| ------------------------------ | ---------------------------------- |
| `DerropsDbClusterEndpoint`     | Aurora cluster writer endpoint     |
| `DerropsDbClusterReadEndpoint` | Aurora cluster reader endpoint     |
| `DerropsDbName`                | Database name (derrops)            |
| `DerropsDbSecretArn`           | ARN of database credentials secret |
| `DerropsDbPort`                | Database port (5432)               |
| `DerropsBastionHostId`         | Bastion host instance ID           |

### Security Group Stack Exports

| Export Name             | Description                      |
| ----------------------- | -------------------------------- |
| `derrops-opensearch-sg` | OpenSearch security group ID     |
| `derrops-rds-sg`        | RDS security group ID            |
| `derrops-backend-sg`    | Lambda backend security group ID |

### Hosted Zone Stack Exports

| Export Name                | Description              |
| -------------------------- | ------------------------ |
| `derrops-hosted-zone-id`   | Private hosted zone ID   |
| `derrops-hosted-zone-name` | Private hosted zone name |
...
| `DerropsApiId`       | API Gateway REST API ID        |
...
const vpcId = cdk.Fn.importValue('derrops-vpc-id')
const vpcCidrBlock = cdk.Fn.importValue('derrops-vpc-cidr-block')
...
const publicSubnetIds = [
  cdk.Fn.importValue('derrops-vpc-subnet-public-a'),
  cdk.Fn.importValue('derrops-vpc-subnet-public-b'),
  cdk.Fn.importValue('derrops-vpc-subnet-public-c'),
]

// Import auth resources
const userPoolId = cdk.Fn.importValue('DerropsUserPoolId')
const userPoolClientId = cdk.Fn.importValue('DerropsUserPoolClientId')
...
const backendSgId = cdk.Fn.importValue('derrops-backend-sg')
```
...
1. Get the bastion host ID:
   ...
   aws cloudformation describe-stacks \
     --stack-name derrops-database-infrastructure \
     --query 'Stacks[0].Outputs[?ExportName==`DerropsBastionHostId`].OutputValue' \
     --output text
   ```
...
4. Connect to database:
   ...
   aws secretsmanager get-secret-value \
     --secret-id derrops/database/credentials \
...
- Zone name (defaults to `${env}.internal.derrops.com`)
...
pnpm --filter @derrops/infra run cdk deploy DerropsVpcStack
...
pnpm --filter @derrops/infra run cdk deploy DerropsSecurityGroupStack
pnpm --filter @derrops/infra run cdk deploy DerropsHostedZoneStack
```
...
pnpm --filter @derrops/infra run cdk deploy DerropsAuthStack
pnpm --filter @derrops/infra run cdk deploy DerropsDatabaseStack
...
LAMBDA_ARN=$(aws cloudformation describe-stacks \
  ...
  --query 'Stacks[0].Outputs[?ExportName==`DerropsLambdaFunctionArn`].OutputValue' \
  --output text)
...
pnpm --filter @derrops/infra run cdk deploy DerropsApiStack \
...
1. **Amplify Backend** (`packages/derrops-backend`) - imports VPC, Auth, Database, Security Groups
...
For issues or questions, contact: Derrops@Derrops.com
