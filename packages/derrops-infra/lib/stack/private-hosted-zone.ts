...
export interface HostedZoneStackProps extends StackProps {
   ...
   * @default '${env}.internal.derrops.com' (where env comes from ENVIRONMENT env var or 'prod')
   */
  zoneName?: string
}

/**
 * Infrastructure stack for Derrops private hosted zone
 *
 * This stack contains:
...
export class HostedZoneStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: HostedZoneStackProps) {
    ...
    Tags.of(this).add('derrops:service', 'dns')
    ...
    const defaultZoneName = `${environment}.internal.derrops.com`
    const zoneName = props?.zoneName ?? defaultZoneName

    this.hostedZone = new route53.PrivateHostedZone(this, 'DerropsHostedZone', {
      zoneName,
      vpc,
    })
    ...
    new CfnOutput(this, 'HostedZoneId', {
      ...
      exportName: 'derrops--platform--dns--hosted-zone-id',
    })

    new CfnOutput(this, 'HostedZoneName', {
      value: this.hostedZone.zoneName,
      description: 'Private hosted zone name',
      exportName: 'derrops--platform--dns--hosted-zone-name',
    })
  }
}
