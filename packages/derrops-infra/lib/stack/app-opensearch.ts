...
/**
 * Infrastructure stack for Derrops OpenSearch Serverless resources
 *
 * This stack contains:
...
export class OpenSearchStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: OpenSearchStackProps) {
    ...
    Tags.of(this).add('derrops:service', 'opensearch')

    const vpcId = Fn.importValue('derrops--platform--vpc--id')

    const privateSubnetIds = [
      Fn.importValue('derrops--platform--vpc--subnet-private-a'),
      Fn.importValue('derrops--platform--vpc--subnet-private-b'),
      Fn.importValue('derrops--platform--vpc--subnet-private-c'),
    ]
    ...
    const opensearchSecurityGroupId = Fn.importValue('derrops--platform--opensearch--sg-id')
    ...
    const vpcEndpoint = new opensearchserverless.CfnVpcEndpoint(this, 'DerropsVpcEndpoint', {
      name: 'derrops--opensearch--vpc-ep',
      vpcId,
    ...
    const collectionName = 'derrops--opensearch'

    const networkPolicy = new opensearchserverless.CfnSecurityPolicy(this, 'DerropsNetworkPolicy', {
      name: 'derrops--opensearch--net-policy',
      type: 'network',
    ...
    this.collection = new opensearchserverless.CfnCollection(this, 'DerropsOpenSearchCollection', {
      name: collectionName,
      type: 'SEARCH',
      description: 'Derrops OpenSearch Serverless collection for log analytics and OASpec search',
    })
    ...
    const dataAccessPolicy = new opensearchserverless.CfnSecurityPolicy(
      this,
      'DerropsDataAccessPolicy',
      {
        name: 'derrops--opensearch--data-access',
        type: 'data',
        policy: JSON.stringify({
          Rules: [
            {
    ...
    this.collection.tags.setTag('derrops:org', 'derrops')
    this.collection.tags.setTag('derrops:domain', 'platform')
    this.collection.tags.setTag('derrops:service', 'opensearch')
    this.collection.tags.setTag('derrops:managed-by', 'cdk')

    new CfnOutput(this, 'OpenSearchCollectionId', {
      value: this.collection.attrId,
      description: 'OpenSearch Serverless collection ID',
      exportName: 'derrops--platform--opensearch--collection-id',
    })

    new CfnOutput(this, 'OpenSearchCollectionName', {
      value: this.collection.name,
      description: 'OpenSearch Serverless collection name',
      exportName: 'derrops--platform--opensearch--collection-name',
    })

    new CfnOutput(this, 'OpenSearchCollectionArn', {
      value: this.collection.attrArn,
      description: 'OpenSearch Serverless collection ARN',
      exportName: 'derrops--platform--opensearch--collection-arn',
    })

    new CfnOutput(this, 'OpenSearchCollectionEndpoint', {
      value: this.collection.attrCollectionEndpoint,
      description: 'OpenSearch Serverless collection endpoint',
      exportName: 'derrops--platform--opensearch--collection-endpoint',
    })
    ...
    new CfnOutput(this, 'OpenSearchDashboardEndpoint', {
      ...
      exportName: 'derrops--platform--opensearch--dashboard-endpoint',
    })

    new CfnOutput(this, 'VpcEndpointId', {
      value: vpcEndpoint.attrId,
      description: 'OpenSearch Serverless VPC endpoint ID',
      exportName: 'derrops--platform--opensearch--vpc-endpoint-id',
    })
  }
}
