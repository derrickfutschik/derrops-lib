...
AWS Amplify Backend for Derrops Platform

## Overview

This package contains the AWS Amplify Gen 2 backend configuration for the Derrops platform. It integrates the NestJS API from `apps/derrops-cloud` and deploys it as a serverless Lambda function with API Gateway, connecting to infrastructure resources (Auth, Database) from `@derrops/infra`.

The actual code for the backend is in the `apps/derrops-cloud` directory. No Application code is in this package, only the infrastructure code for the App Layer.

## Architecture

- **NestJS API as Lambda**: The `derrops-cloud` NestJS app runs in AWS Lambda
- **Infrastructure Integration**: References Cognito and Aurora from `@derrops/infra` via CloudFormation exports
- **No Database Definition**: Database is managed entirely in `@derrops/infra` package
- **No API Gateway**: API Gateway is managed in `@derrops/infra` package, Lambda ARN is exported for reference
...
The NestJS application from `apps/derrops-cloud` is deployed as a Lambda function:

- **Entry point**: `apps/derrops-cloud/src/lambda.ts`
- **Runtime**: Node.js 20
- **Memory**: 512MB
- **Timeout**: 30 seconds
...
The backend imports resources from `@derrops/infra`:
...
- `DerropsLambdaFunctionArn` - Lambda function ARN (for API Gateway integration)
- `DerropsLambdaFunctionName` - Lambda function name

## API Endpoints

The API Gateway is managed in `@derrops/infra` package. Once deployed, the API is available at:

```bash
# Get API URL from infrastructure stack
aws cloudformation describe-stacks \
  --stack-name derrops-api-infrastructure \
  --query 'Stacks[0].Outputs[?ExportName==`DerropsApiEndpoint`].OutputValue' \
  --output text
```
...
**Important**: Do NOT define database resources here. All database resources (Aurora, VPC, networking) are managed in the `@derrops/infra` package and referenced via CloudFormation exports.
