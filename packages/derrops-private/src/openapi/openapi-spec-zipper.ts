  ...
  for (const param of pathParams) {
    ...
    if (param.schema) {
      ...
      if ('$ref' in param.schema) {
        ...
        paramType = typeof param.schema.type === 'string' ? param.schema.type : 'string'
      }
    }
    ...
    variablePath = variablePath.replace(`{${paramName}}`, `{${abbreviatedType}}`)
  }
 ...
 * e.g., P:{i}/logout
 */
function buildPartitionKey(method: string, variablePath: string): string {
  const methodShorthand = getMethodShorthand(method)
  // Remove leading slash from path if present
  const cleanPath = variablePath.startsWith('/') ? variablePath.slice(1) : variablePath
 ...
 */
export function zipOperations(spec: OpenAPIV3_1.Document): ZippedOperation[] {
