...
export async function buildAPIIndex(
  ...
  serverRepo: Repository<IndexedServerDoc>,
) {
  const apiId = buildApiId(specs)

  const serverDocsAll = buildAllServerDocsToBeIndexed(specs, apiId)

  const serverDocs = serverDocsAll.filter(
    (server, index, self) =>
      index ===
      self.findIndex(
        (t) => t.host_template === server.host_template && t.base_path === server.base_path,
      ),
  )
...
export async function buildOperationIndex(
  specs: OpenAPIV3_1.Document,
  operationRepo: Repository<IndexedOperationDoc>,
) {
  const apiId = buildApiId(specs)
 ...
 */
export function buildAllServerDocsToBeIndexed(
 ...
 * @returns
 */
export function buildOperationDocToBeIndexed(
  operation: {
    operation_id: string
 ...
 */
export function buildAllOperationDocsToBeIndexed(
  spec: OpenAPIV3_1.Document,
  apiId: string,
): IndexedOperationDoc[] {
      ...
      if (operation) {
        operations.push(
          buildOperationDocToBeIndexed(
            buildOperationItem({ method, path: path, operation }),
            apiId,
          ),
        )
      }
 ...
 * @returns The API ID
 */
export const buildApiId = (spec: OpenAPIV3_1.Document) =>
  hash(JSON.stringify(findOpenApiIdentifiableAttributes(spec)))
